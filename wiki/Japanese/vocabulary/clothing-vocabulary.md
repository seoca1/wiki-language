---
source: "clothing-vocabulary-jp"
category: "clothing"
level: "JLPT N4-N5"
theme: "clothing"
---

# 衣服・ファッション語彙 (Japanese Clothing & Fashion)

**Source:** [[clothing-vocabulary-jp]]
**Theme:** 衣服とファッション
**Level:** JLPT N4-N5 / A2-B1

日本語衣服・ファッション語彙：基本衣類、色/素材の組み合わせ、生地素材、衣類関連動詞/形容詞を網羅。出典：JLPT N4-N5語彙リスト、日本語教科書。

---

## 基本衣類

### シャツ (shatsu)

**品詞:** 名詞

**定義:** 上半身用の衣類。通常袖、襟、ボタン付き。

**読み / ひらがな / ローマ字:** シャツ (shatsu)

**語源:** 英語 "shirt" から。カタカナ外来語。

#### 例文

- 青い**シャツ**を着ています。 — 描写
- **シャツ**は上の引き出しに。 — 収納

#### 関連語

- ''Tシャツ'' — カジュアルシャツ
- ''ブラウス'' — 女性用シャツ
- ''ポロシャツ'' — 襟付きカジュアル

#### 文化的背景

「シャツを着る」= 準備する。「シャツ一枚」= 身軽な様子。ビジネスではワイシャツ（白無地）が標準。

#### 出典

- [[clothing-vocabulary-jp]]

---

### ズボン (zubon)

**品詞:** 名詞

**定義:** 各脚を別々に覆う、腰から足首までの衣類。

**読み / ひらがな / ローマ字:** ズボン (zubon)

**語源:** フランス語 *jupon* "ペチコート" → 日本語でズボン。またはポルトガル語 *gibão* "上着" からとも。

#### 例文

- この**ズボン**は長すぎる。 — サイズ
- **ズボン**か**パンツ**か。 — 呼称差

#### 関連語

- ''ジーンズ'' — デニムズボン
- ''スラックス'' — フォーマル用
- ''短パン'' — 短いズボン

#### 文化的背景

日本: 「ズボン」= ズボン全般。「ズボンの紐」= 信頼関係。「ズボラ」= だらしない（ズボンが緩い→だらしないの転訛説）。

#### 出典

- [[clothing-vocabulary-jp]]

---

### ドレス (doresu)

**品詞:** 名詞

**定義:** 女性/女児用のワンピース衣類。体と脚を覆う。

**読み / ひらがな / ローマ字:** ドレス (doresu)

**語源:** 英語 "dress" から。古フランス語 *drecier* "整える" → ラテン語 *directus*。

#### 例文

- 赤い**ドレス**を着た。 — 場面
- 夏**ドレス**は軽い。 — 季節

#### 関連語

- ''ワンピース'' — 同義語（日常用）
- ''イブニングドレス'' — 夜会服
- ''ウェディングドレス'' — 結婚式用

#### 文化的背景

「リトルブラックドレス」= 定番アイテム。「ドレスコード」= 服装規定。成人式・卒業式で振袖・ドレス着用。

#### 出典

- [[clothing-vocabulary-jp]]

---

### 靴 (kutsu)

**品詞:** 名詞

**定義:** 足を覆い保護する履物。

**読み / ひらがな / ローマ字:** くつ (kutsu)

**語源:** 古くは「くつ」のみ。漢字「靴」= 「革」+「化」= 革で作ったもの。

#### 例文

- **靴**を脱いでください。 — 室内習慣
- 新しいランニング**靴**。 — 目的別

#### 関連語

- ''スニーカー'' — 運動靴
- ''ブーツ'' — 長靴
- ''サンダル'' — オープンシューズ

#### 文化的背景

室内では靴を脱ぐ習慣（玄関で脱ぐ）。「人の靴を履く」= 共感。「大きな靴を履く」= 大きな期待。

#### 出典

- [[clothing-vocabulary-jp]]

---

### 帽子 (boushi)

**品詞:** 名詞

**定義:** 頭にかぶる覆い。多くはつば付き。

**読み / ひらがな / ローマ字:** ぼうし (boushi)

**語源:** 「帽」= 覆う + 「子」= もの。古くは「冠」= かんむり。

#### 例文

- 日差しに**帽子**を。 — 保護
- **帽子**と手袋。 — 冬セット

#### 関連語

- ''キャップ'' — つばのみ前
- ''ニット帽'' — 編み帽子
- ''麦わら帽子'' — 夏用

#### 文化的背景

「帽子を取る」= 敬意。「帽子をかぶる」= 役職に就く。「帽子屋の帽子なし」= 専門家が自分のことは疎い。

#### 出典

- [[clothing-vocabulary-jp]]

---

### コート (kooto)

**品詞:** 名詞

**定義:** 袖付きの外套。他の服の上に着る。

**読み / ひらがな / ローマ字:** コート (kooto)

**語源:** 英語 "coat" から。古フランス語 *cote* → ゲルマン語 *kotta* "粗布"。

#### 例文

- **コート**を着て。 — 寒さ対策
- **冬コート** vs **レインコート**。 — 種類

#### 関連語

- [[ジャケット]] — 短いコート
- ''トレンチコート'' — 定番スタイル
- ''オーバーコート'' — フォーマル長コート

#### 文化的背景

「コートを脱ぐ」= リラックス。「コートの下」= 隠し事。

#### 出典

- [[clothing-vocabulary-jp]]

---

### スカート (sukaato)

**品詞:** 名詞

**定義:** 腰から吊るし、脚を部分的/完全に覆う衣類。

**読み / ひらがな / ローマ字:** スカート (sukaato)

**語源:** 英語 "skirt" から。古ノルド語 *skyrta* "シャツ" → *shirt* と同源。

#### 例文

- Aライン**スカート**。 — スタイル
- **スカート**丈は膝上。 — 長さ

#### 関連語

- ''ミニスカート'' — 非常に短い
- ''タイトスカート'' — ぴったり
- ''ロングスカート'' — 足首まで

#### 文化的背景

制服スカート = 日本の学校文化。男子も着用（スコットランド・キルト）。「スカートめくり」= 悪戯。

#### 出典

- [[clothing-vocabulary-jp]]

---

### 靴下 (kutsushita)

**品詞:** 名詞

**定義:** 編み物の足覆い。靴の中で履く。

**読み / ひらがな / ローマ字:** くつした (kutsushita)

**語源:** 「靴」+「下」= 靴の下に履くもの。

#### 例文

- 毎日清潔な**靴下**を。 — 衛生
- **靴下**に砂利。 — 不快

#### 関連語

- ''ストッキング'' — 薄手、長い
- ''タイツ'' — 不透明、全脚
- ''アンクルソックス'' — 短い

#### 文化的背景

靴下 = 定番プレゼント（父の日等）。「靴下を履かせる」= 世話を焼く。「穴あき靴下」= 貧乏の象徴。

#### 出典

- [[clothing-vocabulary-jp]]

---

### 手袋 (tebukuro)

**品詞:** 名詞

**定義:** 指ごとに分かれた手の覆い。

**読み / ひらがな / ローマ字:** てぶくろ (tebukuro)

**語源:** 「手」+「袋」= 手の袋。

#### 例文

- 冬に**手袋**を。 — 防寒
- **手袋**を外す。 — 準備完了

#### 関連語

- ''ミトン'' — 親指のみ別
- ''ゴム手袋'' — 医療/使い捨て
- ''運転手袋'' — 革製

#### 文化的背景

「手袋を取る」= 真剣勝負。「白手袋」= 丁寧な扱い。「手袋を投げる」= 決闘の申し込み（西洋）。

#### 出典

- [[clothing-vocabulary-jp]]

---

### マフラー (mafuraa)

**品詞:** 名詞

**定義:** 首や頭に巻く布製品。防寒またはファッション。

**読み / ひらがな / ローマ字:** マフラー (mafuraa)

**語源:** 英語 "muffler" "マフラー（消音器）" と同語源 → 首を覆うもの→マフラー。

#### 例文

- 赤い**マフラー**を巻く。 — アクセサリー
- **マフラー**と帽子セット。 — 冬

#### 関連語

- ''ネックウォーマー'' — 筒状
- ''ストール'' — 大判
- ''スヌード'' — 輪状

#### 文化的背景

マフラー = 冬の定番ギフト。応援マフラー = チーム忠誠。「マフラーを巻く」= 用心深くなる。

#### 出典

- [[clothing-vocabulary-jp]]

---

## 色・素材の組み合わせ

### 赤いドレス (akai doresu)

**品詞:** 連語

**定義:** 赤色のドレス。

**読み / ひらがな / ローマ字:** あかいドレス (akai doresu)

#### 例文

- パーティーに**赤いドレス**で。 — 場面
- **赤いドレス**が目立つ。 — 視覚

#### 関連語

- [[ドレス]] — 基本衣類
- ''赤'' — 色

#### 出典

- [[clothing-vocabulary-jp]]

---

### 青いシャツ (aoi shatsu)

**品詞:** 連語

**定義:** 青色のシャツ。

**読み / ひらがな / ローマ字:** あおいシャツ (aoi shatsu)

#### 例文

- 仕事に**青いシャツ**で。 — ユニフォーム
- **青いシャツ**にベージュパンツ。 — 組み合わせ

#### 関連語

- [[シャツ]] — 基本衣類
- ''青'' — 色

#### 出典

- [[clothing-vocabulary-jp]]

---

### 白いズボン (shiroi zubon)

**品詞:** 連語

**定義:** 白色のズボン。

**読み / ひらがな / ローマ字:** しろいズボン (shiroi zubon)

#### 例文

- 夏は**白いズボン**で。 — 季節
- **白いズボン**は汚れやすい。 — メンテ

#### 関連語

- [[ズボン]] — 基本衣類
- ''白'' — 色

#### 出典

- [[clothing-vocabulary-jp]]

---

### 黒い靴 (kuroi kutsu)

**品詞:** 連語

**定義:** 黒色の靴。

**読み / ひらがな / ローマ字:** くろいくつ (kuroi kutsu)

#### 例文

- **黒い靴**を磨く。 — 手入れ
- スーツに**黒い靴**。 — フォーマル

#### 関連語

- ''靴'' — 基本衣類
- ''黒'' — 色

#### 出典

- [[clothing-vocabulary-jp]]

---

## 生地素材

### 絹 (kinu)

**品詞:** 名詞

**定義:** 蚕の繭から採れる細く強い繊維。高級織物用。

**読み / ひらがな / ローマ字:** きぬ (kinu)

**語源:** 古くは「きぬ」のみ。漢字「絹」= 「糸」+「系」= 糸の系統。

#### 例文

- **絹**のネクタイ。 — アクセサリー
- **絹**は滑らか。 — 触感

#### 関連語

- ''蚕'' — 由来
- ''絹織物'' — 織物
- ''着物'' — 伝統衣装

#### 文化的背景

絹 = 日本の伝統産業（群馬・長野・山梨）。シルクロード = 交易路。「絹の如し」= 滑らか。正絹 = 純絹。

#### 出典

- [[clothing-vocabulary-jp]]

---

### 綿 (men/wata)

**品詞:** 名詞

**定義:** 綿花から採れる柔らかい白色繊維。布地用。

**読み / ひらがな / ローマ字:** めん (men) / わた (wata)

**語源:** 中国語 "mián" → 日本語 "めん"。和名 "わた" = "綿"の訓読み。

#### 例文

- 100%**綿**シャツ。 — 表示
- **綿**は通気性が良い。 — 特性

#### 関連語

- ''綿花'' — 原料
- ''オーガニックコットン'' — 持続可能
- ''デニム'' — 綿ツイル

#### 文化的背景

木綿 = 江戸時代の普及品（海島綿→和綿）。「綿入れ」= 綿入れ半纏。「綿密」= 細かい。

#### 出典

- [[clothing-vocabulary-jp]]

---

### 羊毛 (youmou)

**品詞:** 名詞

**定義:** 羊や他動物から採れる繊維。

**読み / ひらがな / ローマ字:** ようもう (youmou)

**語源:** 「羊」+「毛」= 羊の毛。漢語。

#### 例文

- **羊毛**セーター。 — 保温
- **メリノウール**。 — 高級

#### 関連語

- ''羊'' — 由来
- ''カシミヤ'' — 山羊毛
- ''メリノ'' — 羊品種

#### 文化的背景

ウール = 天然断熱材。「ウール100%」= 高品質。「毛布」= 羊毛製。

#### 出典

- [[clothing-vocabulary-jp]]

---

## 衣類動詞・形容詞

### 着る (kiru)

**品詞:** 動詞 (一段)

**定義:** 衣類やアクセサリーを身に着ける。

**読み / ひらがな / ローマ字:** きる (kiru)

**語源:** 古日本語 *kiru* "着る、切る"。同音「切る」と区別。

#### 例文

- 何を**着ますか**？ — 選択
- **着て**行きます。 — 動作

#### 関連語

- ''着せる'' — 他動詞（着せる）
- ''脱ぐ'' — 反対
- ''着飾る'' — おしゃれする

#### 出典

- [[clothing-vocabulary-jp]]

---

### 脱ぐ (nugu)

**品詞:** 動詞 (五段・ガ行)

**定義:** 身体から衣類を脱ぐ。

**読み / ひらがな / ローマ字:** ぬぐ (nugu)

**語源:** 古日本語 *nugu* "脱ぐ、拭く"。

#### 例文

- **靴を脱いで**ください。 — 指示
- **上着を脱ぐ**。 — リラックス

#### 関連語

- ''着る'' — 反対
- ''脱ぎ捨てる'' — 乱雑に脱ぐ
- ''着替える'' — 服を変える

#### 出典

- [[clothing-vocabulary-jp]]

---

### 洗う (arau)

**品詞:** 動詞 (五段・ワ行)

**定義:** 水と洗剤で衣類を洗浄する。

**読み / ひらがな / ローマ字:** あらう (arau)

**語源:** 古日本語 *arau* "洗う、荒う"。

#### 例文

- **水洗い**してください。 — 表示
- **色落ち**注意で**洗う**。 — 色ケア

#### 関連語

- ''洗濯'' — 名詞（洗うこと）
- ''ドライクリーニング'' — 代替
- ''シミ'' — 汚れ

#### 出典

- [[clothing-vocabulary-jp]]

---

### 新しい (atarashii)

**品詞:** 形容詞 (イ形容詞)

**定義:** 最近作られ、買われ、得られたもの。未使用。

**読み / ひらがな / ローマ字:** あたらしい (atarashii)

**語源:** 「新た」+「しい」= 新たな様子。

#### 例文

- **新しい**靴は痛い。 — 慣らし
- **真新しい**。 — 強調

#### 関連語

- ''新品'' — 強調
- ''中古'' / ''古着'' — 反対
- ''ヴィンテージ'' — 古いが価値あり

#### 出典

- [[clothing-vocabulary-jp]]

---

### 古い (furui)

**品詞:** 形容詞 (イ形容詞)

**定義:** 長く存在する；新しくない。

**読み / ひらがな / ローマ字:** ふるい (furui)

**語源:** 古日本語 *furui* "古い、経た"。

#### 例文

- **古い**お気に入りセーター。 — 思い入れ
- **古着**で寄付。 — 処分

#### 関連語

- ''ヴィンテージ'' — おしゃれな古い
- ''着古した'' — 使用感あり
- ''アンティーク'' — 非常に古い、収集対象

#### 出典

- [[clothing-vocabulary-jp]]

---

### 高い (takai)

**品詞:** 形容詞 (イ形容詞)

**定義:** 価格が高い；高価。

**読み / ひらがな / ローマ字:** たかい (takai)

**語源:** 古日本語 *takashi* "高い、貴い"。

#### 例文

- そのコートは**高い**。 — 価格
- **高級**志向。 — 好み

#### 関連語

- ''高価'' — 同義語（硬い）
- ''お高い'' — 丁寧/婉曲
- ''安い'' — 反対
- ''高級品'' — カテゴリ

#### 出典

- [[clothing-vocabulary-jp]]

---

### 安い (yasui)

**品詞:** 形容詞 (イ形容詞)

**定義:** 価格が低い；安価。

**読み / ひらがな / ローマ字:** やすい (yasui)

**語源:** 「安」+「い」= 安価な様子。

#### 例文

- **安い**Tシャツ。 — 予算
- **安物買いの銭失い**。 — 諺

#### 関連語

- ''格安'' — 中立/ポジティブ
- ''お手頃'' — ポジティブ
- ''高い'' — 反対
- ''掘り出し物'' — 掘り出し物

#### 出典

- [[clothing-vocabulary-jp]]

---

### 綺麗 (kirei)

**品詞:** 形容動詞 (ナ形容詞)

**定義:** 感覚や心に美しく響く；清潔。

**読み / ひらがな / ローマ字:** きれい (kirei)

**語源:** 「奇麗」= 稀で美しい。または「綺」= 整う + 「麗」= 美しい。

#### 例文

- **綺麗な**ドレス。 — 賞賛
- **綺麗な**生地。 — 品質

#### 関連語

- ''美しい'' — よりフォーマル
- ''素敵'' — おしゃれ
- ''汚い'' — 反対

#### 出典

- [[clothing-vocabulary-jp]]

---

### きつい (kitsui)

**品詞:** 形容詞 (イ形容詞)

**定義:** 体にぴったりフィットする；緩くない。

**読み / ひらがな / ローマ字:** きつい (kitsui)

**語源:** 「気」+「強い」= 気が強い → 締め付けが強い。

#### 例文

- このジーンズ**きつい**。 — フィット
- **きつい**スケジュール。 — 比喩

#### 関連語

- ''ぴったり'' — 快適にフィット
- ''タイト'' — 洋服用語
- ''緩い'' — 反対
- ''スキニー'' — 非常にきつい（ジーンズ）

#### 出典

- [[clothing-vocabulary-jp]]

---

### 緩い (yurui)

**品詞:** 形容詞 (イ形容詞)

**定義:** きつくフィットしない；だぶだぶ。

**読み / ひらがな / ローマ字:** ゆるい (yurui)

**語源:** 「緩む」の連体形 → 形容詞化。

#### 例文

- **緩い**セーター。 — 快適
- **緩い**お金。 — 比喩

#### 関連語

- ''だぶだぶ'' — 非常に緩い
- ''オーバーサイズ'' — 意図的に緩い
- ''きつい'' — 反対
- ''リラックスフィット'' — スタイル用語

#### 出典

- [[clothing-vocabulary-jp]]

---

### 長袖 (nagasode)

**品詞:** 名詞

**定義:** 手首まで届く袖。

**読み / ひらがな / ローマ字:** ながそで (nagasode)

#### 例文

- **長袖**シャツ。 — スタイル
- **長袖**をまくる。 — 調整

#### 関連語

- ''半袖'' — 反対
- ''七分袖'' — 中間
- ''ノースリーブ'' — 袖なし

#### 出典

- [[clothing-vocabulary-jp]]

---

### 半袖 (hansode)

**品詞:** 名詞

**定義:** 肘上で終わる袖。

**読み / ひらがな / ローマ字:** はんそで (hansode)

#### 例文

- 夏は**半袖**で。 — 季節
- **半袖**ボタンダウン。 — スタイル

#### 関連語

- ''長袖'' — 反対
- ''Tシャツ'' — 典型的半袖
- ''ポロシャツ'' — 半袖襟付き

#### 出典

- [[clothing-vocabulary-jp]]

---

## 出典

- JLPT N4-N5 語彙リスト
- 日本語衣類ガイド
- [[first-travel-korea]] — 韓国旅行中の衣類単語

---

## Pipeline Form (machine-readable)

> Generated for downstream consumers (`Game/typing_language/raw/{lang}_words.md`).
> Schema reference: `wiki/pipeline-to-game.md` L33-39, L92.
> The body above remains the human-readable form and is the source of truth.

```yaml
- { id: jp_clothing_001, display: "シャツ", input: "シャツ", meaning: "셔츠", level: "N5", category: "clothing-vocabulary", source: "[[clothing-vocabulary-jp]]" }
- { id: jp_clothing_002, display: "ズボン", input: "ズボン", meaning: "바지", level: "N5", category: "clothing-vocabulary", source: "[[clothing-vocabulary-jp]]" }
- { id: jp_clothing_003, display: "ドレス", input: "ドレス", meaning: "드레스", level: "N5", category: "clothing-vocabulary", source: "[[clothing-vocabulary-jp]]" }
- { id: jp_clothing_004, display: "靴", input: "靴", meaning: "구두", level: "N5", category: "clothing-vocabulary", source: "[[clothing-vocabulary-jp]]" }
- { id: jp_clothing_005, display: "帽子", input: "帽子", meaning: "모자", level: "N5", category: "clothing-vocabulary", source: "[[clothing-vocabulary-jp]]" }
- { id: jp_clothing_006, display: "コート", input: "コート", meaning: "코트", level: "N5", category: "clothing-vocabulary", source: "[[clothing-vocabulary-jp]]" }
- { id: jp_clothing_007, display: "スカート", input: "スカート", meaning: "스커트", level: "N5", category: "clothing-vocabulary", source: "[[clothing-vocabulary-jp]]" }
- { id: jp_clothing_008, display: "靴下", input: "靴下", meaning: "양말", level: "N5", category: "clothing-vocabulary", source: "[[clothing-vocabulary-jp]]" }
- { id: jp_clothing_009, display: "手袋", input: "手袋", meaning: "장갑", level: "N5", category: "clothing-vocabulary", source: "[[clothing-vocabulary-jp]]" }
- { id: jp_clothing_010, display: "マフラー", input: "マフラー", meaning: "목도리", level: "N5", category: "clothing-vocabulary", source: "[[clothing-vocabulary-jp]]" }
- { id: jp_clothing_011, display: "赤いドレス", input: "赤いドレス", meaning: "빨간 드레스", level: "N5", category: "clothing-vocabulary", source: "[[clothing-vocabulary-jp]]" }
- { id: jp_clothing_012, display: "青いシャツ", input: "青いシャツ", meaning: "파란 셔츠", level: "N5", category: "clothing-vocabulary", source: "[[clothing-vocabulary-jp]]" }
- { id: jp_clothing_013, display: "白いズボン", input: "白いズボン", meaning: "하얀 바지", level: "N5", category: "clothing-vocabulary", source: "[[clothing-vocabulary-jp]]" }
- { id: jp_clothing_014, display: "黒い靴", input: "黒い靴", meaning: "검은 구두", level: "N5", category: "clothing-vocabulary", source: "[[clothing-vocabulary-jp]]" }
- { id: jp_clothing_015, display: "絹", input: "絹", meaning: "실크", level: "N4", category: "clothing-vocabulary", source: "[[clothing-vocabulary-jp]]" }
- { id: jp_clothing_016, display: "綿", input: "綿", meaning: "면", level: "N4", category: "clothing-vocabulary", source: "[[clothing-vocabulary-jp]]" }
- { id: jp_clothing_017, display: "羊毛", input: "羊毛", meaning: "양모", level: "N4", category: "clothing-vocabulary", source: "[[clothing-vocabulary-jp]]" }
- { id: jp_clothing_018, display: "革", input: "革", meaning: "가죽", level: "N4", category: "clothing-vocabulary", source: "[[clothing-vocabulary-jp]]" }
- { id: jp_clothing_019, display: "着る", input: "着る", meaning: "입다", level: "N5", category: "clothing-vocabulary", source: "[[clothing-vocabulary-jp]]" }
- { id: jp_clothing_020, display: "脱ぐ", input: "脱ぐ", meaning: "벗다", level: "N5", category: "clothing-vocabulary", source: "[[clothing-vocabulary-jp]]" }
- { id: jp_clothing_021, display: "洗う", input: "洗う", meaning: "세탁하다", level: "N5", category: "clothing-vocabulary", source: "[[clothing-vocabulary-jp]]" }
- { id: jp_clothing_022, display: "新しい", input: "新しい", meaning: "새로운", level: "N5", category: "clothing-vocabulary", source: "[[clothing-vocabulary-jp]]" }
- { id: jp_clothing_023, display: "古い", input: "古い", meaning: "오래된", level: "N5", category: "clothing-vocabulary", source: "[[clothing-vocabulary-jp]]" }
- { id: jp_clothing_024, display: "高い", input: "高い", meaning: "비싼", level: "N4", category: "clothing-vocabulary", source: "[[clothing-vocabulary-jp]]" }
- { id: jp_clothing_025, display: "安い", input: "安い", meaning: "저렴한", level: "N5", category: "clothing-vocabulary", source: "[[clothing-vocabulary-jp]]" }
- { id: jp_clothing_026, display: "綺麗", input: "綺麗", meaning: "아름다운", level: "N4", category: "clothing-vocabulary", source: "[[clothing-vocabulary-jp]]" }
- { id: jp_clothing_027, display: "きつい", input: "きつい", meaning: "꼬인", level: "N4", category: "clothing-vocabulary", source: "[[clothing-vocabulary-jp]]" }
- { id: jp_clothing_028, display: "緩い", input: "緩い", meaning: "넓은", level: "N4", category: "clothing-vocabulary", source: "[[clothing-vocabulary-jp]]" }
- { id: jp_clothing_029, display: "長袖", input: "長袖", meaning: "긴소매", level: "N4", category: "clothing-vocabulary", source: "[[clothing-vocabulary-jp]]" }
- { id: jp_clothing_030, display: "半袖", input: "半袖", meaning: "반소매", level: "N4", category: "clothing-vocabulary", source: "[[clothing-vocabulary-jp]]" }
```