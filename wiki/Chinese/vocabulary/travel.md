---
source: "first-travel-china"
category: "travel"
level: "HSK 1-3"
theme: "travel"
---

# 旅行 (lǚxíng) - Travel

**Source:** [[transportation-zh]]
**Theme:** Travel & Tourism
**Level:** HSK 1-3

中国国内旅行核心词汇。涵盖机场、酒店、餐厅、交通、景点、问路六大场景。所有名词标注量词，拼音标声调，HSK等级标注。

---

## 1. 机场 (jīchǎng) - Airport

#### 名词

| 中文 | 拼音 | 量词 | HSK | 英文 | 备注 |
|------|------|------|-----|------|------|
#### 机场

### 机场

| 机场 | jīchǎng | 个 | 2 | airport | |
#### 护照

### 护照

| 护照 | hùzhào | 本 | 2 | passport | |
#### 机票

| 机票 | jīpiào | 张 | 2 | plane ticket | |
#### 登机牌

| 登机牌 | dēngjīpái | 张 | 3 | boarding pass | |
#### 行李

### 行李

| 行李 | xíngli | 件 | 2 | luggage | |
#### 托运

| 托运 | tuōyùn | 件 | 3 | checked baggage | 动词/名词均可 |
#### 手提行李

| 手提行李 | shǒutí xíngli | 件 | 3 | carry-on | |
#### 登机口

### 登机口

| 登机口 | dēngjīkǒu | 个 | 3 | gate | |
#### 航站楼

| 航站楼 | hángzhànlóu | 个 | 3 | terminal | |
#### 跑道

| 跑道 | pǎodào | 条 | 3 | runway | |
#### 值机

### 值机

| 值机 | zhíjī | 次 | 3 | check-in | 动词/名词 |

#### 动词

| 中文 | 拼音 | HSK | 英文 |
|------|------|-----|------|
#### 值机

| 值机 | zhíjī | 3 | check in |
#### 登机

| 登机 | dēngjī | 3 | board |
#### 降落

| 降落 | jiàngluò | 3 | land |
#### 起飞

| 起飞 | qǐfēi | 3 | take off |
#### 到达

| 到达 | dàodá | 2 | arrive |
#### 出发

| 出发 | chūfā | 2 | depart |

#### 核心表达

- **请出示护照和登机牌。** — Please show passport and boarding pass.
- **登机口在哪里？** — Where is the gate?
- **我的行李丢了。** — My luggage is lost.
- **这班航班延误了。** — This flight is delayed.

---

## 2. 酒店 (jiǔdiàn) - Hotel

#### 名词

| 中文 | 拼音 | 量词 | HSK | 英文 | 备注 |
|------|------|------|-----|------|------|
#### 酒店

### 酒店

| 酒店 | jiǔdiàn | 家 | 2 | hotel | |
#### 预订

### 预订

| 预订 | yùdìng | 个 | 3 | reservation | 动词/名词 |
#### 入住

### 入住

| 入住 | rùzhù | 次 | 3 | check-in | 动词/名词 |
#### 退房

| 退房 | tuìfáng | 次 | 3 | check-out | 动词/名词 |
#### 房间

### 房间

| 房间 | fángjiān | 间 | 1 | room | |
#### 单人间

| 单人间 | dānrénjiān | 间 | 3 | single room | |
#### 双人间

| 双人间 | shuāngrénjiān | 间 | 3 | double/twin room | |
#### 前台

| 前台 | qiántái | 个 | 3 | front desk | |
#### 大堂

| 大堂 | dàotáng | 个 | 3 | lobby | |
#### 电梯

| 电梯 | diàntī | 台 | 2 | elevator | |
#### 房卡

| 房卡 | fángkǎ | 张 | 3 | key card | |
#### 无线网

| 无线网 | wúxiànwǎng | 个 | 3 | Wi-Fi | 口语说 "wifi密码" |
#### 早餐

| 早餐 | zǎocān | 份 | 2 | breakfast | |

#### 动词

| 中文 | 拼音 | HSK | 英文 |
|------|------|-----|------|
#### 预订

| 预订 | yùdìng | 3 | reserve/book |
#### 入住

| 入住 | rùzhù | 3 | check in |
#### 退房

| 退房 | tuìfáng | 3 | check out |
#### 住

| 住 | zhù | 1 | stay |

#### 核心表达

- **我有预订，叫 [名字]。** — I have a reservation under [name].
- **请帮我办理入住。** — Please check me in.
- **Wi-Fi密码是什么？** — What's the Wi-Fi password?
- **早餐包含吗？** — Is breakfast included?

---

## 3. 餐厅 (cāntīng) - Restaurant

#### 名词

| 中文 | 拼音 | 量词 | HSK | 英文 | 备注 |
|------|------|------|-----|------|------|
#### 餐厅

### 餐厅

| 餐厅 | cāntīng | 家 | 2 | restaurant | |
#### 菜单

### 菜单

| 菜单 | càidān | 本/份 | 2 | menu | |
#### 点菜

### 点菜

| 点菜 | diǎncài | 次 | 2 | order | 动词/名词 |
#### 推荐

| 推荐 | tuījiàn | 个 | 3 | recommendation | 动词/名词 |
#### 菜

| 菜 | cài | 道 | 2 | dish | |
#### 水

| 水 | shuǐ | 杯/瓶 | 1 | water | |
#### 米饭

| 米饭 | mǐfàn | 碗 | 1 | rice | |
#### 筷子

| 筷子 | kuàizi | 双 | 2 | chopsticks | |
#### 勺子

| 勺子 | sháozi | 把 | 2 | spoon | |
#### 买单

### 买单

| 买单 | mǎidān | 次 | 2 | pay the bill | 动词/名词 |
#### 发票

| 发票 | fāpiào | 张 | 3 | receipt/invoice | |
#### 服务员

| 服务员 | fúwùyuán | 位/个 | 2 | waiter | |

#### 动词

| 中文 | 拼音 | HSK | 英文 |
|------|------|-----|------|
#### 点

| 点 | diǎn | 2 | order |
#### 吃

| 吃 | chī | 1 | eat |
#### 喝

| 喝 | hē | 1 | drink |
#### 推荐

| 推荐 | tuījiàn | 3 | recommend |
#### 买单

| 买单 | mǎidān | 2 | pay the bill |
#### 尝

| 尝 | cháng | 3 | taste |

#### 核心表达

- **我要这个。** — I'll have this one.
- **有什么推荐？** — What do you recommend?
- **来杯水。** — Water, please.
- **买单。** — The bill, please.
- **能刷卡吗？** — Can I pay by card?

---

## 4. 交通 (jiāotōng) - Transportation

#### 名词

| 中文 | 拼音 | 量词 | HSK | 英文 | 备注 |
|------|------|------|-----|------|------|
#### 地铁

### 地铁

| 地铁 | dìtiě | 条/号线 | 2 | subway | 说 "坐几号线" |
#### 公交车

| 公交车 | gōngjiāochē | 辆/路 | 2 | bus | |
#### 出租车

### 出租车

| 出租车 | chūzūchē | 辆 | 2 | taxi | 也叫 "的士" (粤语区) |
#### 火车

| 火车 | huǒchē | 趟/列 | 2 | train | |
#### 高铁

### 高铁

| 高铁 | gāotiě | 趟/列 | 3 | high-speed rail | |
#### 车站

### 车站

| 车站 | chēzhàn | 个 | 2 | station | |
#### 站

| 站 | zhàn | 个 | 2 | stop/station | 地铁站/公交站 |
#### 票

| 票 | piào | 张 | 2 | ticket | |
#### 交通卡

| 交通卡 | jiāotōngkǎ | 张 | 3 | transport card | 各城市不同 (北京一卡通/深圳通等) |
#### 换乘

### 换乘

| 换乘 | huànchéng | 次 | 3 | transfer | |
#### 线路

| 线路 | xiànlù | 条 | 3 | line/route | |
#### 方向

| 方向 | fāngxiàng | 个 | 2 | direction | |
#### 出口

| 出口 | chūkǒu | 个 | 2 | exit | 地铁站出口编号 A/B/C/D... |

#### 动词

| 中文 | 拼音 | HSK | 英文 |
|------|------|-----|------|
#### 坐

| 坐 | zuò | 1 | ride/take |
#### 下

| 下 | xià | 1 | get off |
#### 换乘

| 换乘 | huànchéng | 3 | transfer |
#### 走

| 走 | zǒu | 1 | walk |
#### 去

| 去 | qù | 1 | go |

#### 核心表达

- **这趟车去 [地点] 吗？** — Does this go to [place]?
- **地铁站在哪儿？** — Where's the subway station?
- **一张票到 [地点]。** — One ticket to [place].
- **几号出口出去？** — Which exit do I take?

---

## 5. 景点 (jǐngdiǎn) - Sightseeing

#### 名词

| 中文 | 拼音 | 量词 | HSK | 英文 | 备注 |
|------|------|------|-----|------|------|
#### 旅游

| 旅游 | lǚyóu | 次/个 | 2 | travel/tourism | 动词/名词 |
#### 景点

### 景点

| 景点 | jǐngdiǎn | 个 | 3 | attraction | |
#### 地图

### 地图

| 地图 | dìtú | 张/份 | 2 | map | |
#### 导游

| 导游 | dǎoyóu | 位/个 | 3 | guide | |
#### 团

| 团 | tuán | 个 | 3 | tour group | 跟团/自由行 |
#### 博物馆

| 博物馆 | bówùguǎn | 家/座 | 3 | museum | |
#### 寺庙

| 寺庙 | sìmiào | 座/座 | 3 | temple | |
#### 公园

| 公园 | gōngyuán | 个 | 2 | park | |
#### 山

| 山 | shān | 座 | 1 | mountain | |
#### 海

| 海 | hǎi | 片/个 | 1 | sea | |
#### 海滩

| 海滩 | hǎitān | 个 | 3 | beach | |
#### 门票

### 门票

| 门票 | ménpiào | 张 | 3 | admission ticket | |
#### 照片

| 照片 | zhàopiàn | 张 | 2 | photo | |
#### 纪念品

| 纪念品 | jìniànpǐn | 个/件 | 3 | souvenir | |

#### 动词

| 中文 | 拼音 | HSK | 英文 |
|------|------|-----|------|
#### 旅游

| 旅游 | lǚyóu | 2 | travel/sightsee |
#### 参观

| 参观 | cānguān | 3 | visit (place) |
#### 逛

| 逛 | guàng | 2 | walk around/shop |
#### 拍照

| 拍照 | pāizhào | 2 | take photos |
#### 买

| 买 | mǎi | 1 | buy |

#### 核心表达

- **门票多少钱？** — How much is the ticket?
- **能给张地图吗？** — Can I have a map?
- **这儿能拍照吗？** — Can I take photos here?
- **这个多少钱？** — How much is this?

---

## 6. 问路 (wènlù) - Asking Directions

#### 名词

| 中文 | 拼音 | 量词 | HSK | 英文 | 备注 |
|------|------|------|-----|------|------|
#### 路

| 路 | lù | 条 | 1 | road/way | |
#### 方向

| 方向 | fāngxiàng | 个 | 2 | direction | |
#### 左边

### 左边

| 左边 | zuǒbiān | 个 | 1 | left | |
#### 右边

### 右边

| 右边 | yòubiān | 个 | 1 | right | |
#### 直走

### 直走

| 直走 | zhízǒu | 词 | 2 | straight | 动词短语 |
#### 十字路口

| 十字路口 | shízìlùkǒu | 个 | 3 | intersection | |
#### 红绿灯

| 红绿灯 | hónglǜdēng | 个 | 2 | traffic light | |
#### 斑马线

| 斑马线 | bānmǎxiàn | 条 | 3 | crosswalk | |
#### 街区

| 街区 | jiēqū | 个 | 3 | block | |
#### 附近

### 附近

| 附近 | fùjìn | 处/个 | 2 | nearby | |
#### 远

| 远 | yuǎn | — | 2 | far | 形容词 |

#### 核心表达

- **请问，[地点] 在哪儿？** — Excuse me, where is [place]?
- **去 [地点] 怎么走？** — How do I get to [place]?
- **直走。** — Go straight.
- **左拐 / 右拐。** — Turn left / right.
- **近吗？** — Is it close?
- **远不远？** — Is it far?

---

## 文化提示

#### 1. 中国交通卡
- 各城市有自己的卡：北京「一卡通」、上海「交通卡」、深圳「深圳通」、广州「羊城通」
- 地铁、公交、共享单车、便利店通用
- 可在机场/地铁站/便利店充值退卡

#### 2. 中国餐厅文化
- 茶水/纸巾通常免费或收少量费 (1-2元)
- 筷子/勺子免费提供
- 结账说「买单」或「服务员」
- 微信/支付宝扫码支付最普及，现金次之，刷卡较少
- 小费文化：不给小费，高档餐厅可能含服务费 10-15%

#### 3. 酒店/住宿
- 入住通常 14:00，退房 12:00
- 身份证登记入住 (实名制)
- 民宿/青旅/快捷酒店/星级酒店选择多
- 携程/美团/Booking/飞猪 主要预订平台

#### 4. 移动支付必备
- **支付宝** / **微信支付** 双核心
- 绑定银行卡或充值余额
- 扫商家码 (主扫) 或出示付款码 (被扫)
- 红包/转账/信用卡还款/生活缴费 一站式

#### 5. 打车软件
- **滴滴出行** (主流)，曹操出行、T3出行、高德打车等聚合平台
- 选车型：快车/专车/豪华/拼车/出租车
- 机场/火车站有专门上客区

#### 6. 紧急电话
- 报警：110
- 急救/火警：119
- 交通事故：122
- 旅游投诉：12301

---

## Sources

- [[transportation-zh]] — 中国国内交通/旅行交通基础词汇
- [[directions-zh]] — 问路/方向词汇补充
- [[body-zh]] — 日常生活基础词汇补充

---

### 支付宝

### 微信支付

### 滴滴

## Pipeline Form (machine-readable)

> Generated for downstream consumers (`Game/typing_language/raw/{lang}_words.md`).
> Schema reference: `wiki/pipeline-to-game.md` L33-39, L92.
> The body above remains the human-readable form and is the source of truth.

```yaml
- { id: 'zh_travel_001', display: '机场', input: '机场', meaning: '', level: 'HSK 1-3', category: 'travel', source: '[[first-travel-china]]' }
- { id: 'zh_travel_002', display: '护照', input: '护照', meaning: '', level: 'HSK 1-3', category: 'travel', source: '[[first-travel-china]]' }
- { id: 'zh_travel_003', display: '行李', input: '行李', meaning: '', level: 'HSK 1-3', category: 'travel', source: '[[first-travel-china]]' }
- { id: 'zh_travel_004', display: '登机口', input: '登机口', meaning: '', level: 'HSK 1-3', category: 'travel', source: '[[first-travel-china]]' }
- { id: 'zh_travel_005', display: '值机', input: '值机', meaning: '', level: 'HSK 1-3', category: 'travel', source: '[[first-travel-china]]' }
- { id: 'zh_travel_006', display: '酒店', input: '酒店', meaning: '', level: 'HSK 1-3', category: 'travel', source: '[[first-travel-china]]' }
- { id: 'zh_travel_007', display: '预订', input: '预订', meaning: '', level: 'HSK 1-3', category: 'travel', source: '[[first-travel-china]]' }
- { id: 'zh_travel_008', display: '入住', input: '入住', meaning: '', level: 'HSK 1-3', category: 'travel', source: '[[first-travel-china]]' }
- { id: 'zh_travel_009', display: '房间', input: '房间', meaning: '', level: 'HSK 1-3', category: 'travel', source: '[[first-travel-china]]' }
- { id: 'zh_travel_010', display: '餐厅', input: '餐厅', meaning: '', level: 'HSK 1-3', category: 'travel', source: '[[first-travel-china]]' }
- { id: 'zh_travel_011', display: '菜单', input: '菜单', meaning: '', level: 'HSK 1-3', category: 'travel', source: '[[first-travel-china]]' }
- { id: 'zh_travel_012', display: '点菜', input: '点菜', meaning: '', level: 'HSK 1-3', category: 'travel', source: '[[first-travel-china]]' }
- { id: 'zh_travel_013', display: '买单', input: '买单', meaning: '', level: 'HSK 1-3', category: 'travel', source: '[[first-travel-china]]' }
- { id: 'zh_travel_014', display: '地铁', input: '地铁', meaning: '', level: 'HSK 1-3', category: 'travel', source: '[[first-travel-china]]' }
- { id: 'zh_travel_015', display: '出租车', input: '出租车', meaning: '', level: 'HSK 1-3', category: 'travel', source: '[[first-travel-china]]' }
- { id: 'zh_travel_016', display: '高铁', input: '高铁', meaning: '', level: 'HSK 1-3', category: 'travel', source: '[[first-travel-china]]' }
- { id: 'zh_travel_017', display: '车站', input: '车站', meaning: '', level: 'HSK 1-3', category: 'travel', source: '[[first-travel-china]]' }
- { id: 'zh_travel_018', display: '换乘', input: '换乘', meaning: '', level: 'HSK 1-3', category: 'travel', source: '[[first-travel-china]]' }
- { id: 'zh_travel_019', display: '景点', input: '景点', meaning: '', level: 'HSK 1-3', category: 'travel', source: '[[first-travel-china]]' }
- { id: 'zh_travel_020', display: '地图', input: '地图', meaning: '', level: 'HSK 1-3', category: 'travel', source: '[[first-travel-china]]' }
- { id: 'zh_travel_021', display: '门票', input: '门票', meaning: '', level: 'HSK 1-3', category: 'travel', source: '[[first-travel-china]]' }
- { id: 'zh_travel_022', display: '左边', input: '左边', meaning: '', level: 'HSK 1-3', category: 'travel', source: '[[first-travel-china]]' }
- { id: 'zh_travel_023', display: '右边', input: '右边', meaning: '', level: 'HSK 1-3', category: 'travel', source: '[[first-travel-china]]' }
- { id: 'zh_travel_024', display: '直走', input: '直走', meaning: '', level: 'HSK 1-3', category: 'travel', source: '[[first-travel-china]]' }
- { id: 'zh_travel_025', display: '附近', input: '附近', meaning: '', level: 'HSK 1-3', category: 'travel', source: '[[first-travel-china]]' }
- { id: 'zh_travel_026', display: '支付宝', input: '支付宝', meaning: '', level: 'HSK 1-3', category: 'travel', source: '[[first-travel-china]]' }
- { id: 'zh_travel_027', display: '微信支付', input: '微信支付', meaning: '', level: 'HSK 1-3', category: 'travel', source: '[[first-travel-china]]' }
- { id: 'zh_travel_028', display: '滴滴', input: '滴滴', meaning: '', level: 'HSK 1-3', category: 'travel', source: '[[first-travel-china]]' }
```
