# Chinese Learning Wiki - Index

Last updated: 2026-08-11 (24 new culture pages + 4 new expression themes [requests/complaints/emotions-reactions/small-talk] — culture count 19 → 43 = Spanish parity, expression count 13 [5-lang parity ✓])

> Chinese (zh) is the **5th language** in the vault, expanding the prior 4-language footprint (English / Japanese / Korean / Spanish). This index follows the same pattern as the other languages' indexes; section headers and theme-file conventions are inherited from `Language/schema/AGENTS.md` (lines 267-294) and the `theme-file` convention established in the 2026-07-10 lint session (no per-word / per-expression `.md` files).

## Vocabulary

- [[time-vocabulary]] - Chinese 시간 어휘 (today/tomorrow/morning/afternoon/evening/night/weekend/hour) (13 theme files)

The Chinese vocabulary section landed its first 5 theme files on 2026-07-13 from the OpenClaw raw batch. All entries carry a Pipeline Form YAML appendix at the bottom of each file (5-field schema + `source` anchor per `wiki/pipeline-to-game.md` L33-39). All entries preserve the OpenClaw pinyin notation (tone marks), with numbered-pinyin variants in the YAML `input` field so the game pipeline can pick whichever form it needs.

- [[body-zh]] — 신체 부위 (身体部位). 23 words (HSK 1-2).
- [[colors-zh]] — 색깔 (颜色). 11 words (HSK 1-2).
- [[family-zh]] — 가족 호칭 (家庭成员). 23 words (HSK 1-2).
- [[measure-words-zh]] — 양사 (量词). 11 words (HSK 1-2).
- [[numbers-zh]] — 숫자 (数字). 16 words (HSK 1).
- [[time-zh]] — 시간 (时间). 일상 시간 표현 (HSK 1-2).
- [[weather-zh]] — 날씨 (天气). 기상 + 계절 (HSK 1-2).
- [[education-zh]] — 교육 (教育). 학교/학습 어휘 (HSK 2-3).
- [[transportation-zh]] — 교통 (交通). 교통수단 + 길찾기 (HSK 1-2).
- [[directions-zh]] — 방향 (方向). 동/서/남/북 + 위치 표현 (HSK 1-2).
- [[weekdays-zh]] — 요일 (星期). 월~일 (HSK 1).
- [[months-zh]] — 월 (月份). 1월~12월 + 계절 (HSK 1).
- [[health-zh]] — 건강 (健康). 병원/증상/약 (HSK 2-3).
- [[colors-zh]] — 색깔 (颜色). 11 words (HSK 1-2).
- [[family-zh]] — 가족 호칭 (家庭成员). 11 words (HSK 1-2).
- [[measure-words-zh]] — 양사 (量词). 11 words (HSK 1-2).
- [[numbers-zh]] — 숫자 (数字). 12 words (HSK 1).
- [[body-zh]] — 신체 부위 (身体部位). 11 words (HSK 1-2).
- [[time-zh]] - 时间 (Time) — 일상 시간 표현 (HSK 1-2)
- [[weather-zh]] - 天气 (Weather) — 기상 + 계절 (HSK 1-2)
- [[education-zh]] - 教育 (Education) — 학교/학습 어휘 (HSK 2-3)
- [[education-vocabulary]] - 교육 어휘 (multi-language comparison)
- [[ordinal-numbers-zh]] - 序数 (Ordinal numbers) — 第一/第二/第三... 第 prefix + cardinal number (HSK 1-2)
- [[technology-zh]] - 科技 (Technology) — 电脑/手机/互联网/邮件/网站/密码 (HSK 2-3)
- [[zh-adjectives-vocabulary]] - 形容词 — 中文 기본 형용사 (大/小/高/矮/新/旧/好/坏/快/慢/多/少/...) (HSK 1-3)
- [[zh-daily-life-vocabulary]] - 日常生活动词 — 起床/睡觉/吃饭/喝水/去/来/工作/学习/... (HSK 1-3)
- [[zh-time-prepositions-vocabulary]] - 时间 + 介词 — 今天/明天/昨天/早上/中午/晚上/在/到/从/向/给/跟 (HSK 1-3)
- [[zh-polite-expressions-vocabulary]] - 礼貌表达 — 请/谢谢/对不起/麻烦/不客气/... (HSK 1-3)
- [[zh-restaurant-vocabulary]] - 餐厅词汇 — 点菜/菜单/结账/推荐/预订/打包/外卖 (HSK 2-4)

Planned future theme families (mirroring the other languages):

- Travel — 旅行 — airport, hotel, restaurant, transportation, directions
- Food & Restaurant — 食物与餐厅 — ingredients, dishes, beverages, dining
- Business — 商务 — email, meetings, corporate vocabulary
- Emotions & Personality — 情绪与性格 — feelings, traits, verbs
- Nature & Weather — 自然与天气 — phenomena, landforms, plants
- Animals — 动物 — pets, wild animals, insects, marine life
- Clothing & Fashion — 服装与时尚 — garments, materials, colors
- [[time-zh]] - 时间 (Time) — 일상 시간 표현 (HSK 1-2)
- [[weather-zh]] - 天气 (Weather) — 기상 + 계절 (HSK 1-2)
- [[education-zh]] - 教育 (Education) — 학교/학습 어휘 (HSK 2-3)
- [[education-vocabulary]] - 교육 어휘 (multi-language comparison)

## Expressions

- [[common-phrases]] - Chinese 핵심 일상 표현 (greeting/clarification/request/question) (5 theme files / ~60 entries)

The Chinese expressions section was expanded on 2026-07-19 from scaffold state to 5 theme files. All follow the theme-file convention (one theme file per category, with per-expression `## {entry}` sections) and contain bilingual (zh-KO) phrasebook with usage notes.

- [[daily-basics]] — 매일 기본 표현 (15 핵심 표현 + 紧急 호출)
- [[dating-romance]] — 연애 표현 (12 표현, 缘分, 相亲, 老公/老婆)
- [[business-basics]] — 비즈니스 표현 (10 표현, 报价, 合同, 发票, 请多关照)
- [[travel-basics]] — 여행 표현 (10 표현, 机场, 出租车, 酒店, 报警)
- [[requests]] — 请求 표현 — 请、麻烦你、能不能、可以吗、拜托、您能...吗、请帮我、能不能麻烦你、劳驾、求你了 (10 표현, A2-B1; 您/你 honorific distinction noted)
- [[complaints]] — 投诉/抱怨表达 — 我要投诉、这不是我点的、我对这个服务不满意、你能修一下吗、这有问题 (B1-C1; polite escalation patterns)
- [[emotions-reactions]] — 情绪反应 — 真的?, 哇, 太棒了!, 我不敢相信, 不会吧, 哎呀, 真的吗, 太可惜了 (10 표현, A1-A2; 4성조 modulation noted)
- [[small-talk]] — 闲聊话题 — 今天天气真好, 你吃了吗, 你在哪儿工作, 你家是哪里的, 最近忙不忙, 周末有什么安排 (8 표현, A2-B1; safe topics: weather/food/work/regional origin)
- [[food-dining]] — 식당 표현 (10 표현, 菜单, 买单, 不要辣, 我吃素)

## Culture (43 entries; 34 added 2026-08-11)

The Chinese culture section was expanded on 2026-07-19 with 4 comprehensive essay-length pages.

- [[chinese-dating-culture]] — Chinese Dating Culture — 相亲, 90/00后 세대, 결혼 시장 변화
- [[chinese-cuisine-culture]] — Chinese Cuisine Culture — 8대 菜系, 차 문화, 테이블 매너, 현대 트렌드
- [[family-and-filial-piety]] — Chinese Family & Filial Piety — 孝, 421困境, 4-2-1 구조, 가족 명절
- [[chinese-workplace-culture]] — Chinese Workplace Culture — Guanxi, Mianzi, Banquet, 996, N-po Generation, Civil Service Exam
- [[chinese-communication-mianzi]] - 中国 面子 沟通 — 面子、关系、老师、孝、微信 (2026-08-10 추가)
- [[chinese-food-culture]] - 中国 饮食 文化 — 八大菜系、火锅、敬酒、功夫茶 (2026-08-10 추가)
- [[chinese-modern-life]] - 中国 现代 生活 — 微信、支付宝、美团、高铁、抖音 (2026-08-10 추가)
- [[chinese-new-year-traditions]] - 中国 新年 传统 — 春节、红包、年夜饭、饺子、春运 (2026-08-10 추가)
- [[chinese-workplace-guanxi]] - 中国 职场 关系 — 关系、996、躺平、内卷、老板 (2026-08-10 추가)
- [[chinese-family-structure]] - 中国 家庭 结构 — 计划生育、421困境、独生子女、家族 vs 家庭 (2026-08-11 추가)
- [[chinese-education-system]] - 中国 教育 制度 — 九年义务教育、高考、211/985大学、中考小升初、应试教育 (2026-08-11 추가)
- [[chinese-sports-culture]] - 中国 体育 文化 — 乒乓球国球、武术、功夫、太极拳、中超、跳水 (2026-08-11 추가)
- [[chinese-religious-holidays]] - 中国 传统 节日 — 春节、中秋、端午、清明、重阳、七夕 (2026-08-11 추가)
- [[chinese-regional-variations]] - 中国 地域 差异 — 普通话 vs 粤/闽/吴、8大菜系、南北差异 (2026-08-11 추가)
- [[chinese-food-history]] - 中国 饮食 历史 — 筷子、茶文化、8大菜系、火锅、功夫茶 (2026-08-11 추가)
- [[chinese-pop-culture]] - 中国 流行 文化 — 古风、网络文学、C-드라마、抖音、流量明星 (2026-08-11 추가)
- [[chinese-history-trivia]] - 中国 历史 常识 — 春秋战国、丝绸之路、四大发明、长城、汉字演变 (2026-08-11 추가)
- [[chinese-arts-traditions]] - 中国 传统 艺术 — 国画、书法、京剧、茶艺、中医 (2026-08-11 추가)
- [[chinese-tech-workplace]] - 中国 科技 职场 — 996/007、华为/阿里/腾讯/字节跳动、内卷、躺平、老板 (2026-08-11 추가)

### Added 2026-08-11 (24 new festival / life-cycle / social / traditional-arts pages — Spanish parity)

- [[chinese-chunjie]] — 春节 (Spring Festival) — 红包 / 年夜饭 / 春晚 / 春联 / 饺子
- [[chinese-zhongqiu]] — 中秋节 (Mid-Autumn) — 月饼 / 赏月 / 嫦娥 / 桂花 / 团圆
- [[chinese-duanwu]] — 端午节 (Dragon Boat) — 粽子 / 龙舟 / 屈原 / 五色丝线 / 雄黄酒
- [[chinese-qingming]] — 清明节 (Tomb-Sweeping) — 扫墓 / 踏青 / 青团 / 寒食 / 介子推
- [[chinese-yuanxiao]] — 元宵节 (Lantern) — 元宵 / 灯谜 / 花灯 / 舞龙 / 守岁
- [[chinese-chongyang]] — 重阳节 (Double Ninth) — 登高 / 敬老 / 菊花 / 重阳糕 / 王维诗
- [[chinese-qixi]] — 七夕 (Qixi, Chinese Valentine) — 牛郎织女 / 乞巧 / 巧果 / 鹊桥
- [[chinese-zodiac]] — 十二生肖 (12 Zodiac) — 鼠牛虎兔龙蛇马羊猴鸡狗猪 + 本命年
- [[chinese-wedding-traditions]] — 中国 婚礼 — 拜堂 / 闹洞房 / 敬茶 / 彩礼 / 红色主题
- [[chinese-funeral-traditions]] — 中国 丧礼 — 守孝三年 / 烧纸 / 灵堂 / 头七 / 出殡
- [[chinese-red-envelope]] — 红包 (Hóngbāo) — 压岁钱 / 微信红包 / 吉利金额 / 婚礼礼金
- [[chinese-banquet-seating]] — 中国 圆桌 席位 — 主客位 / 敬酒顺序 / 干杯 / 鱼头朝首席
- [[chinese-tobacco-alcohol]] — 中国 烟酒 文化 — 茅台 / 中华烟 / 干杯 / 拼酒 / 茅台五粮液
- [[chinese-guanxi]] — 关系 (Guānxi) — 关系 vs 朋友 / 请客 / 人情 / 圈子 / 走后门
- [[chinese-mianzi]] — 面子 (Miànzi) — 给面子 / 丢面子 / 长面子 / 看面子 / 体面
- [[chinese-bamboo-culture]] — 竹 文化 — 岁寒三友 / 竹笋 / 文房 / 苏轼诗 / 君子象征
- [[chinese-fengshui]] — 风水 (Fēngshuǐ) — 阴阳五行 / 罗盘 / 坐北朝南 / 财位 / 八卦镜
- [[chinese-calligraphy]] — 书法 (Shūfǎ) — 文房四宝 / 王羲之 / 笔法 / 气韵 / 力透纸背
- [[chinese-medicine-tcm]] — 中医 (TCM) — 阴阳五行 / 经络 / 针灸 / 中药 / 四诊
- [[chinese-wushu-martial-arts]] — 武术 (Wǔshù) — 套路 / 太极拳 / 少林 / 武当 / 长拳散打
- [[chinese-opera-regional]] — 中国 戏曲 — 京剧 / 粤剧 / 昆曲 / 川剧变脸 / 黄梅戏
- [[chinese-five-script-styles]] — 五种 字体 — 篆 / 隶 / 楷 / 行 / 草 + 王羲之
- [[chinese-confucius-temple]] — 孔庙 (Kǒngmiào) — 祭孔大典 / 曲阜 / 衍圣公 / 历代科举
- [[chinese-hutong-culture]] — 北京 胡同 — 四合院 / 邻里 / 照壁 / 南锣鼓巷 / 蒙古语词源

## Sources (21 processed)

The Chinese sources section absorbed lesson files from `.openclaw/workspace/wiki/chinese/lessons/` on 2026-07-13 (4 files) and expanded with culture source pages on 2026-07-19 (5 files). Each source page follows the Source Summary format from `Language/schema/AGENTS.md` lines 225-265 (Type / Date Added / Language Level + Summary / Key Takeaways / Vocabulary Extracted / Expressions Extracted / Cultural Insights / Notes).

### Lesson Sources (4)

- [[pinyin-basics-zh]] — Pinyin Basics (Pinyin_Basics.md). HSK 1 beginner. Lesson on the romanization system (21 initials + 38 finals + 4 tones).
- [[tone-pairs-zh]] — Tone Pairs (Tone_Pairs_Chinese.md). HSK 1 beginner. Tone sandhi rules (3+3, 一, 不).
- [[greetings-zh]] — Greetings and Self-Introduction (Greetings_Chinese.md). HSK 1 beginner. 20 greetings + 是 (shì) self-intro pattern.
- [[daily-routine-zh]] — Daily Routine in Chinese (Daily_Routine_Chinese.md). HSK 1-2 beginner. Time + verb pattern + 17 routine verbs.

### Vocabulary Sources (13 new)

- [[numbers-zh]] — Chinese Numbers Vocabulary (16 entries: 1-10, large units, compound numbers, special rules). HSK 1-2.
- [[colors-zh]] — Chinese Colors Vocabulary (11 entries: basic colors). HSK 1-2.
- [[family-zh]] — Chinese Family Vocabulary (23 entries: immediate family, siblings, grandparents, relatives, children, spouse terms). HSK 1-2.
- [[body-zh]] — Chinese Body Parts Vocabulary (23 entries: 머리/얼굴, 상체, 하체). HSK 1-2.
- [[time-zh]] — Chinese Time Vocabulary (시간 단위, 날짜/요일, 시간 표현, 빈도/반복). HSK 1-2.
- [[weather-zh]] — Chinese Weather Vocabulary (기본 날씨, 기온/온도, 계절, 강수/강설, 바람/대기, 날씨 예보, 기상 경보). HSK 1-2.
- [[education-zh]] — Chinese Education Vocabulary (학교/교육기관, 학년/학기, 수업/과목, 교실/시설, 인물, 학습 동사/표현, 성적/평가). HSK 2-3.
- [[transportation-zh]] — Chinese Transportation Vocabulary (교통수단, 장소/시설, 동작/표현, 길찾기/방향, 교통 관련 질문/표현). HSK 1-2.
- [[directions-zh]] — Chinese Directions Vocabulary (기본 방향, 상대적 위치, 방향 동작, 위치 전치사, 거리/범위, 길찾기 표현, 이동/이동수단 방향). HSK 1-2.
- [[weekdays-zh]] — Chinese Weekdays Vocabulary (기본 요일, 요일 관련 표현, 주/주말 관련 어휘, 요일 약어, 문화적 배경). HSK 1.
- [[months-zh]] — Chinese Months Vocabulary (12개월, 월 관련 표현, 계절과 월, 음력/양력, 월별 별칭). HSK 1.
- [[health-zh]] — Chinese Health Vocabulary (병원/의료기관, 의료진, 증상/질병, 신체 부위, 의료 동작/처치, 약/처방, 의료 표현/문장, 문화적 배경). HSK 2-3.

### Culture & Grammar Sources (4)

- [[basic-particles-zh]] — Basic Particles (的/了/在/有). HSK 1-2. 소유격/완료/위치/소유 4종 + 한국어 비교.
- [[word-order-zh]] — Word Order (语序). HSK 1-2. SVO 구조 + 한국어 SOV 와의 차이.
- [[chinese-family-zh]] — Chinese Family Source. 가족 문화, 孝道, 421 family.
- [[chinese-food-culture-zh]] — Chinese Food Culture Source. 8대 菜系, 차 문화.

## Grammar (6 entries; 4 added 2026-08-11)

The Chinese grammar section was initialized on 2026-07-13 with 2 entries absorbed from `.openclaw/workspace/wiki/chinese/grammar/`. Expanded on 2026-08-11 with 4 new entries to reach Spanish parity (6 grammar pages). Each page is HSK-graded beginner-to-intermediate level with detailed Korean learner notes (한국어 학습자 주의 / 한국 한자음 ≠ 중국 병음 warning), Spanish/Japanese/Korean cross-language comparison table, and 简/繁 notes.

- [[basic-particles]] — 기본 조사 (的/了/在/有) — 소유격/완료/위치/소유 4종 + 한국어 비교 + 5단계 학습법 + 실전 회화 (HSK 1-2)
- [[word-order]] — 어순 (Word Order, 语序) — SVO 구조 + 한국어 SOV 와의 차이 + 20 흔한 어순 패턴 + 의문문/부정 (HSK 1-2)
- [[chinese-aspect-le-guo]] — 体貌 了/过/着 — 완료/경험/진행 + 한국 한자음 ≠ 중국 발음 + 4개 언어 비교표 (HSK 2-3)
- [[chinese-modal-verbs]] — 情态动词 能/会/可以/要 — 능력/기술/허가/의지 + 8개 예문 + 한중일서 비교 (HSK 1-3)
- [[chinese-measure-words]] — 量词 个/位/只/条/张/本/件 — 분류사 시스템 + 两 vs 二 + 通用量词 (HSK 1-3)
- [[chinese-ba-sentence]] — 把字句 — 处置式 구문 + 결과보어 필수 + SVO 대비 + 8개 예문 (HSK 3-4)

## Conventions

This wiki follows the same conventions as the other Language wikis:

- **Theme-file convention**: vocabulary and expressions live as theme files (e.g. `food-vocabulary.md`), with per-word / per-expression sections inside. No single-word or single-sentence `.md` files.
- **Pipeline Form YAML**: every vocabulary page carries the game-pipeline YAML appendix (5 fields: `display` / `input` / `meaning` / `level` / `category` + `source`) so the typing game at `Game/typing_language/` can ingest via theme-anchor citations.
- **Wikilinks**: Obsidian stem matching; cross-references should use `[[wikilink]]` form.
- **raw/ is read-only**: the `raw/Chinese/` directory is owned by upstream sources. This wiki writes only to `wiki/Chinese/`.
- **log.md**: every change appends a `## [YYYY-MM-DD] {kind} | {summary}` entry to `log.md` for parseability.

## See also

- `Language/schema/AGENTS.md` — Index Format spec (lines 267-294)
- `Language/wiki/English/index.md` — English index (reference pattern)
- `Language/wiki/Korean/index.md` — Korean index (reference pattern)
- `Language/wiki/Spanish/index.md` — Spanish index (most complete pattern)
- `Language/wiki/content-lineage.md` — content lineage across languages
- `.openclaw/workspace/wiki/Chinese/` — OpenClaw runtime workspace for Chinese (cross-project pipeline)

## Round 2 — Index Reconciliation (2026-07-30)

> Orphan pages reconciled from filesystem. Descriptions from each file's Overview section.

### Expressions (added 3)
- [[greetings]] — Chinese 问候表达 — 你好、初次见面、您好 (5+ entries)
- [[apologies]] — Chinese 道歉表达 — 对不起、抱歉、请原谅 (5+ entries)
- [[agreement]] — Chinese 同意表达 — 好、是的、可以、没错 (5+ entries)

### Vocabulary (added 19; 8 new 2026-08-10)
- [[family-vocabulary]] — Chinese 家庭词汇 — 父母/兄弟姐妹/亲戚 (5+ entries)
- [[colors-vocabulary]] — Chinese 颜色词汇 — 基本颜色表达 (5+ entries)
- [[months-vocabulary]] — Chinese 月份词汇 — 一月~十二月 + 季节 (5+ entries)
- [[technology-vocabulary]] — Chinese 技术词汇 — 计算机/互联网/数字 (5+ entries)
- [[ordinal-numbers-vocabulary]] — Chinese 序数词汇 — 第一、第二、第三... (5+ entries)
- [[weekdays-vocabulary]] — Chinese 星期词汇 — 周一~周日 (5+ entries)
- [[weather-vocabulary]] — Chinese 天气词汇 — 晴/雨/雪/阴 + 气象表达 (5+ entries)
- [[transportation-vocabulary]] — Chinese 交通词汇 — 汽车/地铁/公交/出租车 (5+ entries)
- [[directions-vocabulary]] — Chinese 方向词汇 — 东/西/南/北 + 位置表达 (5+ entries)
- [[health-vocabulary]] — Chinese 健康词汇 — 医院/症状/身体 (5+ entries)
- [[numbers-vocabulary]] — Chinese 数字词汇 — 一~十 + 基本数字 (5+ entries)
- [[sports-zh]] — Chinese 体育运动词汇 — 运动/球类/武术/游泳 (2026-08-10 신규)
- [[shopping-zh]] — Chinese 购物词汇 — 商店/价格/付款/购物动词 (2026-08-10 신규)
- [[holidays-zh]] — Chinese 节日词汇 — 春节/中秋/端午/圣诞 (2026-08-10 신규)
- [[literature-zh]] — Chinese 文学词汇 — 小说/诗歌/戏剧/作家 (2026-08-10 신규)
- [[entertainment-zh]] — Chinese 娱乐词汇 — 电影/电视/音乐/动漫 (2026-08-10 신규)
- [[career-zh]] — Chinese 职业词汇 — 职业/职场/会议/求职 (2026-08-10 신규)
- [[adventure-zh]] — Chinese 冒险词汇 — 登山/露营/游泳/安全 (2026-08-10 신규)
- [[quotes-zh]] — Chinese 名言词汇 — 格言/谚语/经典语录 (2026-08-10 신규)

### Study Plan (added 1)
- [[study-plan/README]] — Chinese study-plan placeholder — meets AGENTS.md 7-subdirectory requirement

## Cross-Language Comparisons

See [[index]] for systematic EN/ES/JP/KR/CH comparisons. Especially relevant for Chinese:

- [[politeness-honorifics]] — 您 vs 你; titles (王老师, 张经理) default address
- [[numbers-counters]] — Mandatory **measure words** (个/位/只/条/张); 两 vs 二 before classifiers
- [[cultural-values]] — **Guanxi** (关系), **Mianzi** (面子), **Yuanfen** (缘分), **Chiku** (吃苦)
- [[family-kinship]] — 爷爷/外公 (paternal vs maternal) split; 表/堂 cousin distinction
- [[food-dining]] — 8大菜系 vs 5-language food comparison
- [[writing-systems]] — Simplified vs Traditional; pinyin; radicals
- [[dating-romance]] — 缘分, 相亲, 老公/老婆 vs EN/ES/JP/KR
- [[mood-systems]] — Chinese modal adverbs (会/要) vs Spanish subjunctive / JP-KR mood suffixes
- [[tense-aspect-systems]] — Chinese aspect particles (了/过) vs tense-heavy systems
- [[diatopic-variation-patterns]] — Chinese 普通话/粤语/闽南语/吴语 (most divergent of 5 langs)
- [[tradiciones-veraniegas]] — Chinese 午休 (wǔxiū, 30-60 min) vs Spanish siesta
- [[lunch-and-rest-patterns]] — Chinese 11:30-12:30 lunch + 午休 culture
- [[master-cheatsheet]] — One-page essential reference per language

- [[sports-comparison]] — 武术 (wushu) heritage; 乒乓球 (ping-pong) cultural symbol; 学校体育 integrated
- [[career-workplace-comparison]] — 996 work culture; 关系 (guanxi) networking; 老板 (laoban) paternalistic
- [[quotes-famous-lines-comparison]] — 古文 (classical) quotations still influential; 武侠 (wuxia) famous lines
- [[entertainment-pop-culture-comparison]] — C-drama + Web novel + 古风 genre; 流量明星 (liúliàng míngxīng = traffic stars)
- [[adventure-outdoor-comparison]] — Mountain pilgrimage (四大佛教名山); 徒步 (hiking) growing; 登山 (dēngshān) heritage
- [[literature-genres-comparison]] — 武侠 (wuxia) + 仙侠 (xianxia); 网络文学 (web literature); 古典 (classical) canon
- [[clothing-fashion-comparison]] — 汉服 (hànfú) revival movement; 唐装 (tangzhuang); 现代 (modern) urban fashion
- [[colors-comparison]] — 五色 (wǔsè = traditional five colors); 红色 (hóngsè) luck/wedding; cultural depth
- [[animals-comparison]] — 十二生肖 (Chinese zodiac); 熊猫 (panda) national symbol; 风水 (fengshui) animal symbolism
- [[family-roles-comparison]] — 辈分 (bèifèn = generation ranking); 长辈 (zhǎngbèi) elders; 关系 (guanxi) extends family
- [[directions-navigation-comparison]] — 东南西北 (dōng-nán-xī-běi); 十字路口 (intersection); addresses hierarchical

## Round 3 — Phase 1.5: Chinese Vocabulary Parity (2026-08-13)

20 English-stem Chinese vocabulary theme files added/expanded to bring Chinese closer to the multi-language parity model (5 themes × 5 languages = cross-language content coverage). Each file follows the established vocabulary theme-file convention (frontmatter + tables + Pipeline Form YAML).

| File | Source stem | Section count | Words |
|------|-------------|---:|---:|
| [[animals-vocabulary]] | animals-vocabulary-zh | 4 | ~25 |
| [[basic-vocabulary]] | basic-vocabulary-zh | 6 | ~50 |
| [[body-vocabulary]] | body-vocabulary-zh | 4 | ~30 |
| [[career-vocabulary]] | career-vocabulary-zh | 4 | ~30 |
| [[clothing-vocabulary]] | clothing-vocabulary-zh | 4 | ~30 |
| [[daily-life-basics]] | daily-life-basics-zh | 4 | ~30 |
| [[emotions-personality-vocabulary]] | emotions-personality-zh | 4 | ~30 |
| [[food-and-dining]] | food-and-dining-zh | 5 | ~40 |
| [[health-and-body]] | health-and-body-zh | 5 | ~40 |
| [[holidays-and-celebrations]] | holidays-and-celebrations-zh | 5 | ~40 |
| [[literature-passages]] | literature-passages-zh | 4 | ~30 |
| [[movie-quotes]] | movie-quotes-zh | 4 | ~30 |
| [[nature-vocabulary]] | nature-vocabulary-zh | 5 | ~40 |
| [[shopping-and-money]] | shopping-and-money-zh | 4 | ~30 |
| [[sports-and-hobbies]] | sports-and-hobbies-zh | 4 | ~30 |
| [[technology-and-internet]] | technology-and-internet-zh | 6 | ~50 |
| [[travel-adventure]] | travel-adventure-zh | 5 | ~40 |
| [[work-and-career]] | work-and-career-zh | 5 | ~40 |
| [[adventure-vocabulary]] | adventure-vocabulary-zh | 5 | ~40 |

Total: 19 new English-stem theme files. The pre-existing `[[quotes-zh]]` (Chinese 名言词汇, 2026-08-10) remains the canonical quotes file (quotes-zh pattern, 545 lines).

**Next steps**: Phase 4 (Expressions Expansion) and Phase 5 (Comparative Wiki Expansion) — cross-language content matrices to follow.
