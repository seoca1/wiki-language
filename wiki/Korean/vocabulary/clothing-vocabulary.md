---
source: "clothing-vocabulary"
category: "clothing"
level: "TOPIK 1-2"
theme: "clothing"
---

# 의류・패션 어휘 (Korean Clothing & Fashion)

**Source:** [[clothing-vocabulary]]
**Theme:** 의류・패션
**Level:** TOPIK 1-2 / A1-B1

한국어 의류・패션 어휘: 상의, 하의, 신발/액세서리, 소재, 색상 의류를 망라. 출처: TOPIK N4-N5 의류 어휘, 국립국어원.

---

## 상의 (Tops)

### 셔츠

**품사:** 명사

**정의:** 상체를 덮는 옷. 보통 칼라, 소매, 단추가 있음.

**로마자 (국립국어원):** syoseo

**한자:** (외래어) 셔츠

#### 예문

- 파란 **셔츠**를 입었습니다. — 복장
- **셔츠**는 위 칸에 넣으세요. — 수납

#### 관련어

- ''티셔츠'' — 캐주얼 셔츠
- ''블라우스'' — 여성용 셔츠
- ''폴로셔츠'' — 칼라 있는 캐주얼

#### 문화적 배경

한국 직장에서 와이셔츠(흰색 무지) 표준. "셔츠 한 장" = 단출한 차림.

#### 출처

- [[clothing-vocabulary]]

---

### 드레스

**품사:** 명사

**정의:** 여성/여아용 원피스 옷. 몸과 다리를 덮음.

**로마자 (국립국어원):** deureseu

**한자:** (외래어) 드레스

#### 예문

- 빨간 **드레스**를 입었다. — 장면
- 여름 **드레스**는 가볍다. — 계절

#### 관련어

- ''원피스'' — 일상용 동의어
- ''이브닝드레스'' — 밤 행사용
- ''웨딩드레스'' — 결혼식용

#### 문화적 배경

한국에서 드레스 = 격식 있는 여성 복장. "드레스 코드" = 복장 규정. 성인식·졸업식에서 한복·드레스 착용.

#### 출처

- [[clothing-vocabulary]]

---

### 코트

**품사:** 명사

**정의:** 소매 있는 외투. 다른 옷 위에 걸침.

**로마자 (국립국어원):** koteu

**한자:** (외래어) 코트

#### 예문

- **코트**를 입으세요. — 추위
- **겨울코트** vs **트렌치코트**. — 종류

#### 관련어

- ''자켓'' — 짧은 코트
- ''트렌치코트'' — 클래식 스타일
- ''오버코트'' — 정장용 긴 코트

#### 문화적 배경

"코트 벗기" = 편해지다. "코트 아래" = 숨긴 것.

#### 출처

- [[clothing-vocabulary]]

---

### 스커트

**품사:** 명사

**정의:** 허리에서 매달려 다리를 부분/전체 덮는 옷.

**로마자 (국립국어원):** seukeo

**한자:** (외래어) 스커트

#### 예문

- H라인 **스커트**. — 스타일
- **스커트** 길이는 무릎 위. — 길이

#### 관련어

- ''미니스커트'' — 매우 짧은
- ''펜슬스커트'' — 타이트
- ''롱스커트'' — 발목까지

#### 문화적 배경

교복 스커트 = 한국 학교 문화. "치마" = 순우리말 동의어.

#### 출처

- [[clothing-vocabulary]]

---

### 가디건

**품사:** 명사

**정의:** 앞이 트인 니트 외투. 단추나 지퍼로 여밈.

**로마자 (국립국어원):** gadian

**한자:** (외래어) 가디건

#### 예문

- **가디건**을 걸쳤습니다. — 착용
- **가디건** 하나 있으면 유용. — 실용성

#### 관련어

- [[니트]] — 니트류 일반
- ''스웨터'' — 풀오버형
- ''카디건'' — 철자 변형

#### 문화적 배경

간절기 필수 아이템. "가디건 걸치다" = 가볍게 입다. 실내 냉방 대비용.

#### 출처

- [[clothing-vocabulary]]

---

### 점퍼

**품사:** 명사

**정의:** 짧은 외투. 지퍼나 단추로 여밈. 방풍·보온용.

**로마자 (국립국어원):** jeompeo

**한자:** (외래어) 점퍼

#### 예문

- **점퍼**를 입으세요. — 추위
- **패딩점퍼** 겨울 필수. — 종류

#### 관련어

- ''자켓'' — 일반 외투
- ''패딩'' — 충전재 있는 점퍼
- ''야상'' — 군용 스타일 점퍼

#### 문화적 배경

한국 겨울 필수템. "점퍼 입다" = 외출 준비. 학생 교복 위 점퍼 착용 흔함.

#### 출처

- [[clothing-vocabulary]]

---

## 하의 (Bottoms)

### 바지

**품사:** 명사

**정의:** 각 다리를 따로 덮는, 허리부터 발목까지의 옷.

**로마자 (국립국어원):** baji

**한자:** (고유어) 바지

#### 예문

- 이 **바지**는 너무 깁니다. — 사이즈
- **바지** 아니면 **치마**. — 선택

#### 관련어

- [[청바지]] — 데님 바지
- ''슬랙스'' — 정장용
- [[반바지]] — 짧은 바지

#### 문화적 배경

한국: "바지" = 바지 전반. "바지런하다" = 부지런하다. "바지 벗다" = 포기하다.

#### 출처

- [[clothing-vocabulary]]

---

### 청바지

**품사:** 명사

**정의:** 데님 천으로 만든 튼튼한 바지.

**로마자 (국립국어원):** cheongbaji

**한자:** "청"(靑) + "바지" = 푸른 바지.

#### 예문

- **청바지**가 잘 어울립니다. — 칭찬
- **청바지** 빨래할 때 뒤집으세요. — 관리

#### 관련어

- ''데님'' — 원단
- ''진'' — jeans 동의어
- ''블랙진'' — 검은 청바지

#### 문화적 배경

청바지 = 캐주얼 필수템. 한국인 1인당 평균 3-4벌 보유. "청바지 입고 출근" = 캐주얼 금기 깨기.

#### 출처

- [[clothing-vocabulary]]

---

### 반바지

**품사:** 명사

**정의:** 무릎 위로 오는 짧은 바지.

**로마자 (국립국어원):** banbaji

**한자:** "반"(半) + "바지" = 반쪽 바지.

#### 예문

- 여름엔 **반바지** 입어요. — 계절
- **반바지** 입고 운동해요. — 활동

#### 관련어

- ''쇼츠'' — 영어 shorts
- ''하프팬츠'' — 무릎 길이
- ''버뮤다팬츠'' — 무릎 약간 위

#### 문화적 배경

한국: "반바지" = 남성도 착용 일반적. 학교 체육복 반바지 표준. "반바지 차림" = 편한 차림.

#### 출처

- [[clothing-vocabulary]]

---

## 신발/액세서리 (Footwear/Accessories)

### 구두

**품사:** 명사

**정의:** 발 전체를 덮는 신발. 가죽·합성소재. 정장용.

**로마자 (국립국어원):** gudu

**한자:** (고유어) 구두

#### 예문

- **구두**를 신으세요. — 정장
- **구두** 닦기 필수. — 관리

#### 관련어

- ''로퍼'' — 끈 없는 구두
- ''옥스포드'' — 끈 있는 정장구두
- ''하이힐'' — 굽 높은 여성 구두

#### 문화적 배경

한국: 실내에서 구두 벗는 문화(현관). "구두 닦이" = 직업/아르바이트. "구두쇠" = 인색한 사람.

#### 출처

- [[clothing-vocabulary]]

---

### 운동화

**품사:** 명사

**정의:** 운동·캐주얼용 고무 밑창 신발.

**로마자 (국립국어원):** undonghwa

**한자:** "운동"(運動) + "화"(靴) = 운동 신발.

#### 예문

- **운동화** 신고 뛰세요. — 운동
- **운동화** 끈 묶기. — 착용

#### 관련어

- ''러닝화'' — 달리기 전용
- ''스니커즈'' — 패션용 운동화
- ''하이탑'' — 발목 덮는 타입

#### 문화적 배경

한국인 1인당 운동화 여러 켤레 보유. "운동화 신고 출근" = 편한 복장 허용. 리셀 시장 활성화.

#### 출처

- [[clothing-vocabulary]]

---

### 모자

**품사:** 명사

**정의:** 머리에 쓰는 것. 챙이나 둘레가 있음.

**로마자 (국립국어원):** moja

**한자:** (고유어) 모자

#### 예문

- **모자**를 쓰세요. — 햇빛
- **모자**와 장갑 세트. — 겨울

#### 관련어

- ''캡'' — 챙만 앞
- ''비니'' — 니트 모자
- ''썬캡'' — 챙 넓은 모자

#### 문화적 배경

"모자 벗다" = 존경. "모자 쓰다" = 역할 맡다. "모자 장수" = 장사꾼.

#### 출처

- [[clothing-vocabulary]]

---

### 양말

**품사:** 명사

**정의:** 발에 신는 니트 직물. 신발 안에서 착용.

**로마자 (국립국어원):** yangmal

**한자:** "양"(洋) + "말"(襪) = 서양 버선.

#### 예문

- 깨끗한 **양말** 매일 신으세요. — 위생
- **양말** 벗고 들어오세요. — 실내

#### 관련어

- ''스타킹'' — 얇고 긴 양말
- ''타이즈'' — 불투명 전신
- ''발목양말'' — 짧은 양말

#### 문화적 배경

양말 = 명절 선물 단골. "양말 신다" = 준비하다. "구멍 난 양말" = 가난 상징.

#### 출처

- [[clothing-vocabulary]]

---

### 장갑

**품사:** 명사

**정의:** 손가락별로 나뉜 손 덮개.

**로마자 (국립국어원):** jangap

**한자:** "장"(掌) + "갑"(甲) = 손의 갑옷.

#### 예문

- 겨울에 **장갑** 끼세요. — 보온
- **장갑** 벗고 일하세요. — 작업

#### 관련어

- ''벙어리장갑'' — 엄지만 분리
- ''고무장갑'' — 의료/주방용
- ''운전장갑'' — 가죽 장갑

#### 문화적 배경

"장갑 끼다" = 준비 완료. "장갑 벗다" = 진심으로 대결. "흰 장갑" = 정중함.

#### 출처

- [[clothing-vocabulary]]

---

### 목도리

**품사:** 명사

**정의:** 목에 두르는 긴 천. 보온 또는 패션.

**로마자 (국립국어원):** mokdoli

**한자:** "목"(頸) + "도리"(수식어) = 목에 두르는 것.

#### 예문

- 빨간 **목도리**를 맸습니다. — 액세서리
- **목도리**와 모자 세트. — 겨울

#### 관련어

- ''머플러'' — 영어 muffler
- ''스카프'' — 얇고 패션용
- ''넥워머'' — 통형

#### 문화적 배경

목도리 = 겨울 필수 선물. 응원 목도리 = 팀 충성심. "목도리 두르다" = 조심스러워지다.

#### 출처

- [[clothing-vocabulary]]

---

## 소재 (Materials)

### 면

**품사:** 명사

**정의:** 목화에서 나는 부드러운 흰 섬유. 직물 원료.

**로마자 (국립국어원):** myeon

**한자:** "면"(綿) = 목화 면.

#### 예문

- 100% **면** 셔츠. — 표시
- **면**은 통기성 좋음. — 특성

#### 관련어

- ''직물'' — 일반
- ''오가닉코튼'' — 친환경
- ''데님'' — 면 트윌

#### 문화적 배경

면직물 = 한국 전통 산업(전주·나주). "면밀하다" = 꼼꼼하다. "면티" = 면 티셔츠.

#### 출처

- [[clothing-vocabulary]]

---

### 니트

**품사:** 명사

**정의:** 실을 고리 잇기 방식으로 짜서 만든 직물.

**로마자 (국립국어원):** niteu

**한자:** (외래어) 니트

#### 예문

- **니트** 스웨터. — 의류
- **니트**는 신축성 있음. — 특성

#### 관련어

- ''스웨터'' — 니트 상의
- ''카디건'' — 앞트임 니트
- ''니트웨어'' — 니트 의류 총칭

#### 문화적 배경

한국: "니트" = 스웨터와 혼용. "니트족" = 니트 즐겨 입는 사람. "손니트" = 손뜨개.

#### 출처

- [[clothing-vocabulary]]

---

### 가죽

**품사:** 명사

**정의:** 동물 가죽을 무두질해 만든 재료.

**로마자 (국립국어원):** gajuk

**한자:** "가죽"(革) = 가죽 혁.

#### 예문

- **가죽** 자켓. — 클래식
- **가죽** 구두. — 정장

#### 관련어

- ''스웨이드'' — 기모 가죽
- ''에나멜가죽'' — 광택 가죽
- ''비건가죽'' — 인조 가죽

#### 문화적 배경

"가죽" = 내구성, 고급. "가죽 점퍼" = 라이더 상징. "가죽 공예" = 전통 공예.

#### 출처

- [[clothing-vocabulary]]

---

### 명주

**품사:** 명사

**정의:** 누에고치에서 얻은 고급 견직물.

**로마자 (국립국어원):** myeongju

**한자:** "명주"(明紬) = 빛나는 명주실.

#### 예문

- **명주** 한복. — 전통
- **명주**는 비쌉니다. — 고급

#### 관련어

- ''실크'' — 영어 표기
- ''비단'' — 동의어 (비단 = 명주)
- ''한복'' — 전통 의상

#### 문화적 배경

명주 = 한국 전통 고급 직물(안동·진주). "명주실" = 고급 실. "비단길" = 실크로드.

#### 출처

- [[clothing-vocabulary]]

---

## 색상 의류 (Colored Clothing)

### 빨간 드레스

**품사:** 관용구

**정의:** 빨간색 드레스.

**로마자 (국립국어원):** ppalgan deureseu

#### 예문

- 파티에 **빨간 드레스** 입고 갔어요. — 장면
- **빨간 드레스**가 눈에 띕니다. — 시각

#### 관련어

- [[드레스]] — 기본 의류
- ''빨간색'' — 색상

#### 출처

- [[clothing-vocabulary]]

---

### 파란 셔츠

**품사:** 관용구

**정의:** 파란색 셔츠.

**로마자 (국립국어원):** palan syoseo

#### 예문

- **파란 셔츠** 입고 출근. — 유니폼
- **파란 셔츠**에 베이지 바지. — 조합

#### 관련어

- [[셔츠]] — 기본 의류
- ''파란색'' — 색상

#### 출처

- [[clothing-vocabulary]]

---

### 하얀 바지

**품사:** 관용구

**정의:** 흰색 바지.

**로마자 (국립국어원):** hayan baji

#### 예문

- 여름에 **하얀 바지** 입어요. — 계절
- **하얀 바지**는 때가 잘 타요. — 관리

#### 관련어

- [[바지]] — 기본 의류
- ''하얀색'' — 색상

#### 출처

- [[clothing-vocabulary]]

---

### 검은 구두

**품사:** 관용구

**정의:** 검은색 구두.

**로마자 (국립국어원):** geomeun gudu

#### 예문

- **검은 구두** 닦으세요. — 관리
- 정장에 **검은 구두** 필수. — 포멀

#### 관련어

- [[구두]] — 기본 신발
- ''검은색'' — 색상

#### 출처

- [[clothing-vocabulary]]

---

## Sources

- TOPIK N4-N5 의류 어휘
- 국립국어원 표준국어대사전
- [[first-travel-japan]] — 한국 여행 중 의류 단어

---

## Pipeline Form (machine-readable)

> Generated for downstream consumers (`Game/typing_language/raw/{lang}_words.md`).
> Schema reference: `wiki/pipeline-to-game.md` L33-39, L92.
> The body above remains the human-readable form and is the source of truth.

```yaml
- { id: ko_clothing_001, display: "셔츠", input: "셔츠", meaning: "shirt", level: "TOPIK 1", category: "clothing-vocabulary", source: "[[clothing-vocabulary]]" }
- { id: ko_clothing_002, display: "드레스", input: "드레스", meaning: "dress", level: "TOPIK 1", category: "clothing-vocabulary", source: "[[clothing-vocabulary]]" }
- { id: ko_clothing_003, display: "코트", input: "코트", meaning: "coat", level: "TOPIK 1", category: "clothing-vocabulary", source: "[[clothing-vocabulary]]" }
- { id: ko_clothing_004, display: "스커트", input: "스커트", meaning: "skirt", level: "TOPIK 1", category: "clothing-vocabulary", source: "[[clothing-vocabulary]]" }
- { id: ko_clothing_005, display: "가디건", input: "가디건", meaning: "cardigan", level: "TOPIK 1", category: "clothing-vocabulary", source: "[[clothing-vocabulary]]" }
- { id: ko_clothing_006, display: "점퍼", input: "점퍼", meaning: "jumper/jacket", level: "TOPIK 1", category: "clothing-vocabulary", source: "[[clothing-vocabulary]]" }
- { id: ko_clothing_007, display: "바지", input: "바지", meaning: "pants", level: "TOPIK 1", category: "clothing-vocabulary", source: "[[clothing-vocabulary]]" }
- { id: ko_clothing_008, display: "청바지", input: "청바지", meaning: "jeans", level: "TOPIK 1", category: "clothing-vocabulary", source: "[[clothing-vocabulary]]" }
- { id: ko_clothing_009, display: "반바지", input: "반바지", meaning: "shorts", level: "TOPIK 1", category: "clothing-vocabulary", source: "[[clothing-vocabulary]]" }
- { id: ko_clothing_010, display: "구두", input: "구두", meaning: "shoes", level: "TOPIK 1", category: "clothing-vocabulary", source: "[[clothing-vocabulary]]" }
- { id: ko_clothing_011, display: "운동화", input: "운동화", meaning: "sneakers", level: "TOPIK 1", category: "clothing-vocabulary", source: "[[clothing-vocabulary]]" }
- { id: ko_clothing_012, display: "모자", input: "모자", meaning: "hat", level: "TOPIK 1", category: "clothing-vocabulary", source: "[[clothing-vocabulary]]" }
- { id: ko_clothing_013, display: "양말", input: "양말", meaning: "socks", level: "TOPIK 1", category: "clothing-vocabulary", source: "[[clothing-vocabulary]]" }
- { id: ko_clothing_014, display: "장갑", input: "장갑", meaning: "gloves", level: "TOPIK 1", category: "clothing-vocabulary", source: "[[clothing-vocabulary]]" }
- { id: ko_clothing_015, display: "목도리", input: "목도리", meaning: "scarf", level: "TOPIK 1", category: "clothing-vocabulary", source: "[[clothing-vocabulary]]" }
- { id: ko_clothing_016, display: "면", input: "면", meaning: "cotton", level: "TOPIK 1", category: "clothing-vocabulary", source: "[[clothing-vocabulary]]" }
- { id: ko_clothing_017, display: "니트", input: "니트", meaning: "knit", level: "TOPIK 1", category: "clothing-vocabulary", source: "[[clothing-vocabulary]]" }
- { id: ko_clothing_018, display: "가죽", input: "가죽", meaning: "leather", level: "TOPIK 1", category: "clothing-vocabulary", source: "[[clothing-vocabulary]]" }
- { id: ko_clothing_019, display: "명주", input: "명주", meaning: "silk", level: "TOPIK 1", category: "clothing-vocabulary", source: "[[clothing-vocabulary]]" }
- { id: ko_clothing_020, display: "빨간 드레스", input: "빨간 드레스", meaning: "red dress", level: "TOPIK 1", category: "clothing-vocabulary", source: "[[clothing-vocabulary]]" }
- { id: ko_clothing_021, display: "파란 셔츠", input: "파란 셔츠", meaning: "blue shirt", level: "TOPIK 1", category: "clothing-vocabulary", source: "[[clothing-vocabulary]]" }
- { id: ko_clothing_022, display: "하얀 바지", input: "하얀 바지", meaning: "white pants", level: "TOPIK 1", category: "clothing-vocabulary", source: "[[clothing-vocabulary]]" }
- { id: ko_clothing_023, display: "검은 구두", input: "검은 구두", meaning: "black shoes", level: "TOPIK 1", category: "clothing-vocabulary", source: "[[clothing-vocabulary]]" }
```