---
source: "entertainment-vocabulary"
category: "entertainment"
level: "TOPIK 3-4"
theme: "entertainment"
---

# 엔터테인먼트 어휘 (Korean Entertainment)

**Source:** [[entertainment-vocabulary]]
**Theme:** 엔터테인먼트/여가
**Level:** TOPIK 3-4 / B1-B2

한국어 엔터테인먼트 어휘: 영화/TV, 음악, 애니메이션/웹툰, 게임/스트리밍, 팬덤 문화. 출처: TOPIK N4-N5, 국립국어원, 콘텐츠진흥원.

---

## 영화/TV

### 영화

**품사:** 명사

**정의:** 화면에映화하는 영상물.

**로마자:** yeonghwa

**한자:** 映畫

#### 예문

- **영화**를 �니다. — 감상
- **영화**가 개봉했습니다. — 상영

#### 관련어

- ''극장'' — theater
- ''감독'' — director

#### 출처

- [[entertainment-vocabulary]]

---

### 배우

**품사:** 명사

**정의:** 연기에 종사하는 사람.

**로마자:** baeu

**한자:** 俳優

#### 예문

- **배우**가 연기합니다. — 연극
- 인기 **배우**의 새 작품. — 인기

#### 관련어

- ''연기자'' — performer
- ''주연'' — lead role

#### 출처

- [[entertainment-vocabulary]]

---

### 드라마

**품사:** 명사

**정의:** 연속극 또는 연극.

**로마자:** deurama

#### 예문

- **드라마**를 시청합니다. — TV
- K-**드라마**가 인기입니다. — 한류

#### 관련어

- ''시리즈'' — series
- ''에피소드'' — episode

#### 출처

- [[entertainment-vocabulary]]

---

### 예능

**품사:** 명사

**정의:** 오락 프로그램.

**로마자:** yeneung

**한자:** 演藝 (예능)

#### 예문

- **예능** 프로그램을 시청합니다. — TV
- **예능**에서 웃음을 찾습니다. — 즐거움

#### 관련어

- ''오락'' — entertainment
- ''버라이어티'' — variety

#### 출처

- [[entertainment-vocabulary]]

---

## 음악

### 음악

**품사:** 명사

**정의:** 소리로 표현하는 예술.

**로마자:** eumak

**한자:** 音樂

#### 예문

- **음악**을 듣습니다. — 감상
- **음악**을 연주합니다. — 연주

#### 관련어

- ''노래'' — song
- ''악기'' — instrument

#### 출처

- [[entertainment-vocabulary]]

---

### 노래

**품사:** 명사

**정의:** 음악적으로 표현된 곡.

**로마자:** norae

#### 예문

- **노래**를 부릅니다. — 가창
- 좋아하는 **노래**를 듣습니다. — 감상

#### 관련어

- ''곡'' — song/track
- ''가사'' — lyrics

#### 출처

- [[entertainment-vocabulary]]

---

### 가수

**품사:** 명사

**정의:** 노래를 부르는 사람.

**로마자:** gasu

**한자:** 歌手

#### 예문

- **가수**의 노래를 듣습니다. — 감상
- 인기 **가수**입니다. — 인기

#### 관련어

- ''싱어'' — singer
- ''아이돌'' — idol

#### 출처

- [[entertainment-vocabulary]]

---

### 콘서트

**품사:** 명사

**정의:** 음악 공연 또는 라이브 무대.

**로마자:** konseoteu

#### 예문

- **콘서트**에 갑니다. — 관람
- **콘서트** 티켓을 예매합니다. — 예매

#### 관련어

- ''공연'' — performance
- ''라이브'' — live

#### 출처

- [[entertainment-vocabulary]]

---

### K-pop

**품사:** 명사

**정의:** 한국 대중음악.

**로마자:** kei-pop

#### 예문

- **K-pop**을 즐깁니다. — 청취
- **K-pop**이 세계적으로 인기가 있습니다. — 한류

#### 관련어

- ''한류'' — Korean Wave
- ''아이돌'' — idol

#### 출처

- [[entertainment-vocabulary]]

---

## 애니메이션/웹툰

### 애니메이션

**품사:** 명사

**정의:** 움직이는 그림 영상물.

**로마자:** aenimeisyeon

#### 예문

- 일본 **애니메이션**을 시청합니다. — 감상
- 한국 **애니메이션** 산업. — 산업

#### 관련어

- ''만화'' — comic
- ''애니'' — anime

#### 출처

- [[entertainment-vocabulary]]

---

### 웹툰

**품사:** 명사

**정의:** 인터넷에서 연재되는 디지털 만화.

**로마자:** wep-tun

#### 예문

- **웹툰**을 읽습니다. — 감상
- **웹툰**이 인기를 끌고 있습니다. — 인기

#### 관련어

- ''만화'' — comic
- ''디지털'' — digital

#### 출처

- [[entertainment-vocabulary]]

---

### 만화

**품사:** 명사

**정의:** 그림으로 표현하는 이야기.

**로마자:** manhwa

**한자:** 漫畫

#### 예문

- 한국 **만화**를 읽습니다. — 감상
- **만화** 카페에 갑니다. — 공간

#### 관련어

- ''웹툰'' — webtoon
- ''코믹'' — comic

#### 출처

- [[entertainment-vocabulary]]

---

## 게임/스트리밍

### 게임

**품사:** 명사

**정의:** 레저/오락 활동으로 행하는 놀이.

**로마자:** geim

#### 예문

- **게임**을 합니다. — 플레이
- 모바일 **게임**을 즐깁니다. — 여가

#### 관련어

- ''플레이'' — play
- ''콘솔'' — console

#### 출처

- [[entertainment-vocabulary]]

---

### 스트리밍

**품사:** 명사

**정의:** 실시간으로 전송되는 미디어 서비스.

**로마자:** seuteuliming

#### 예문

- **스트리밍** 서비스를 이용합니다. — 구독
- 영화 **스트리밍**을 봅니다. — 감상

#### 관련어

- ''실시간'' — real-time
- ''구독'' — subscription

#### 출처

- [[entertainment-vocabulary]]

---

## 팬덤 문화

### 팬

**품사:** 명사

**정의:** 특정 스타/작품을 좋아하는 사람.

**로마자:** paen

#### 예문

- **�**입니다. — 지지
- **팬**미팅에 갑니다. — 만남

#### 관련어

- ''팬덤'' — fandom
- ''덕후'' — fanatic

#### 출처

- [[entertainment-vocabulary]]

---

### 팬덤

**품사:** 명사

**정의:** 특정 팬 커뮤니티.

**로마자:** paendeom

#### 예문

- **팬덤** 활동을 합니다. — 지지
- **팬덤** 문화가 성장하고 있습니다. — 사회

#### 출처

- [[entertainment-vocabulary]]

---

### 아이돌

**품사:** 명사

**정의:** 대중적으로 숭배받는 연예인.

**로마자:** aidol

#### 예문

- K-� **아이돌**을 좋아합니다. — 지지
- **아이돌** 그룹이 데뷔했습니다. — 데뷔

#### 관련어

- ''팬'' — fan
- ''스타'' — star

#### 출처

- [[entertainment-vocabulary]]

---

### 굿즈

**품사:** 명사

**정의:** 팬들이 구매하는 캐릭터 상품.

**로마자:** gutjeu

#### 예문

- **굿즈**를 수집합니다. — 수집
- **굿즈**를 구매합니다. — 소비

#### 관련어

- ''상품'' — merchandise
- ''응원'' — support

#### 출처

- [[entertainment-vocabulary]]

---

## 한중 비교 핵심 정리

| 한국어 | 중국어 | 학습 포인트 |
|---|---|---|
| 영화 | 电影 (diànyǐng) | 한자 电影 동일 |
| 배우 | 演员 (yǎnyuán) | 한자 演员 동일 |
| 드라마 | 电视剧 (diànshìjù) | 한자 电视剧 동일 |
| 음악 | 音乐 (yīnyuè) | 한자 音乐 동일 |
| 노래 | 歌 (gē) | 한자 歌 동일 |
| 가수 | 歌手 (gēshǒu) | 한자 歌手 동일 |
| 콘서트 | 演唱会 (yǎnchànghuì) | 한자 演唱会 동일 |
| 만화 | 漫画 (mànhuà) | 한자 漫画 동일 |
| 게임 | 游戏 (yóuxì) | 한자 游戏 동일 |
| 팬 | 粉丝 (fěnsī) | 음역 |

---

## 출처

- TOPIK N4-N5 교재 어휘 목록
- 국립국어원 표준국어대사전
- 콘텐츠진흥원 (KOCCA) 한국 콘텐츠 산업 통계
- KOFICE (한국국제문화교류진흥원) 한류 백서
- `wiki/comparative/entertainment-pop-culture-comparison.md` (cross-language comparison)

---

## Quick Reference Card

> **한국어 엔터테인먼트 어휘 핵심 15 단어**:
> - 영화 = 영화 (movie)
> - 배우 = 배우 (actor)
> - 드라마 = 드라마 (drama)
> - 예능 = 예능 (variety show)
> - 음악 = 음악 (music)
> - 노래 = 노래 (song)
> - 가수 = 가수 (singer)
> - 콘서트 = 콘서트 (concert)
> - K-pop = K-pop
> - 애니메이션 = 애니메이션 (animation)
> - 웹툰 = 웹툰 (webtoon)
> - 만화 = 만화 (comic)
> - 게임 = 게임 (game)
> - 스트리밍 = 스트리밍 (streaming)
> - 팬 = 팬 (fan)

## Pipeline Form (machine-readable)

> Generated for downstream consumers (`Game/typing_language/raw/{lang}_words.md`).
> Schema reference: `wiki/pipeline-to-game.md` L33-39, L92.
> The body above remains the human-readable form and is the source of truth.

```yaml
- { id: 'kr_entertainment_001', display: '영화', input: '영화', meaning: '', level: 'TOPIK 3-4', category: 'entertainment', source: '[[entertainment-vocabulary]]' }
- { id: 'kr_entertainment_002', display: '배우', input: '배우', meaning: '', level: 'TOPIK 3-4', category: 'entertainment', source: '[[entertainment-vocabulary]]' }
- { id: 'kr_entertainment_003', display: '드라마', input: '드라마', meaning: '', level: 'TOPIK 3-4', category: 'entertainment', source: '[[entertainment-vocabulary]]' }
- { id: 'kr_entertainment_004', display: '예능', input: '예능', meaning: '', level: 'TOPIK 3-4', category: 'entertainment', source: '[[entertainment-vocabulary]]' }
- { id: 'kr_entertainment_005', display: '음악', input: '음악', meaning: '', level: 'TOPIK 3-4', category: 'entertainment', source: '[[entertainment-vocabulary]]' }
- { id: 'kr_entertainment_006', display: '노래', input: '노래', meaning: '', level: 'TOPIK 3-4', category: 'entertainment', source: '[[entertainment-vocabulary]]' }
- { id: 'kr_entertainment_007', display: '가수', input: '가수', meaning: '', level: 'TOPIK 3-4', category: 'entertainment', source: '[[entertainment-vocabulary]]' }
- { id: 'kr_entertainment_008', display: '콘서트', input: '콘서트', meaning: '', level: 'TOPIK 3-4', category: 'entertainment', source: '[[entertainment-vocabulary]]' }
- { id: 'kr_entertainment_009', display: 'K-pop', input: 'K-pop', meaning: '', level: 'TOPIK 3-4', category: 'entertainment', source: '[[entertainment-vocabulary]]' }
- { id: 'kr_entertainment_010', display: '애니메이션', input: '애니메이션', meaning: '', level: 'TOPIK 3-4', category: 'entertainment', source: '[[entertainment-vocabulary]]' }
- { id: 'kr_entertainment_011', display: '웹툰', input: '웹툰', meaning: '', level: 'TOPIK 3-4', category: 'entertainment', source: '[[entertainment-vocabulary]]' }
- { id: 'kr_entertainment_012', display: '만화', input: '만화', meaning: '', level: 'TOPIK 3-4', category: 'entertainment', source: '[[entertainment-vocabulary]]' }
- { id: 'kr_entertainment_013', display: '게임', input: '게임', meaning: '', level: 'TOPIK 3-4', category: 'entertainment', source: '[[entertainment-vocabulary]]' }
- { id: 'kr_entertainment_014', display: '스트리밍', input: '스트리밍', meaning: '', level: 'TOPIK 3-4', category: 'entertainment', source: '[[entertainment-vocabulary]]' }
- { id: 'kr_entertainment_015', display: '팬', input: '팬', meaning: '', level: 'TOPIK 3-4', category: 'entertainment', source: '[[entertainment-vocabulary]]' }
- { id: 'kr_entertainment_016', display: '팬덤', input: '팬덤', meaning: '', level: 'TOPIK 3-4', category: 'entertainment', source: '[[entertainment-vocabulary]]' }
- { id: 'kr_entertainment_017', display: '아이돌', input: '아이돌', meaning: '', level: 'TOPIK 3-4', category: 'entertainment', source: '[[entertainment-vocabulary]]' }
- { id: 'kr_entertainment_018', display: '굿즈', input: '굿즈', meaning: '', level: 'TOPIK 3-4', category: 'entertainment', source: '[[entertainment-vocabulary]]' }
```
