---
source: "nature-vocabulary"
category: "nature"
level: "TOPIK 1-2"
theme: "nature"
---

# 자연・날씨 어휘 (Korean Nature & Weather)

**Source:** [[nature-vocabulary]]
**Theme:** 자연과 날씨
**Level:** TOPIK 1-2 / A1-B1

한국어 자연・날씨 어휘: 천체, 하늘/기상, 자연 풍경, 자연 현상, 계절을 망라. 출처: TOPIK N4-N5 자연 어휘, 국립국어원.

---

## 천체 (Celestial)

### 태양

**품사:** 명사

**정의:** 태양계 중심에 있는 항성. 지구의 빛과 열의 근원.

**로마자 (국립국어원):** taeyang

**한자:** 太陽

#### 예문

- **태양**이 동쪽에서 뜹니다. — 사실
- **태양**을 즐기세요. — 여가

#### 관련어

- ''해돋이'' / ''해넘이'' — 매일의 사건
- ''햇빛'' — 태양빛
- ''태양광'' — 형용사적 용법

#### 문화적 배경

태양 = 많은 문화에서 생명의 원천. "해 뜨다" = 하루 시작. "해 지다" = 하루 끝. "태양처럼 밝다" = 매우 밝음.

#### 출처

- [[nature-vocabulary]]

---

### 달

**품사:** 명사

**정의:** 지구의 천연 위성. 밤에 반사광으로 보임.

**로마자 (국립국어원):** dal

**한자:** 月

#### 예문

- 오늘 밤 **달**이 밝습니다. — 위상
- **달** 아래를 걷다. — 밤

#### 관련어

- ''보름달'' / ''그믐달'' — 위상
- ''달빛'' — 반사광
- ''달나라'' — 동화 속

#### 문화적 배경

달 = 시간의 단위(음력). "달구경" = 추석 풍습. "달을 따다" = 불가능한 일. "달달하다" = 달콤하다.

#### 출처

- [[nature-vocabulary]]

---

### 별

**품사:** 명사

**정의:** 밤하늘의 빛나는 점. 멀리 있는 항성.

**로마자 (국립국어원):** byeol

**한자:** 星

#### 예문

- **별**을 세다. — 밤 활동
- **별**이 쏟아집니다. — 은하수
- **별** 같은 사람. — 은유: 유명인

#### 관련어

- ''별자리'' — 별의 패턴
- ''은하수'' — 별들의 강
- ''별빛'' — 별의 빛

#### 문화적 배경

"별" = 연예인(스타). "별 따러 가자" = 높은 목표. "별일 아니다" = 대수롭지 않다. 칠월칠석 = 견우직녀 별자리 전설.

#### 출처

- [[nature-vocabulary]]

---

## 하늘/기상 (Sky/Weather)

### 하늘

**품사:** 명사

**정의:** 지구에서 본 대기와 우주.

**로마자 (국립국어원):** haneul

**한자:** 空

#### 예문

- **하늘**이 파랗습니다. — 낮
- **하늘**을 날다. — 비행
- **하늘** 높이. — 매우 높음

#### 관련어

- ''하늘색'' — 색상
- ''창공'' — 시적 표현
- ''지하'' — 반대

#### 문화적 배경

"하늘" = 하늘, 천국(종교). "하늘이 무너져도 솟아날 구멍 있다" = 희망. "하늘 같은 은혜" = 큰 은혜.

#### 출처

- [[nature-vocabulary]]

---

### 구름

**품사:** 명사

**정의:** 대기 중 보이는 물방울/얼음 결정 덩어리.

**로마자 (국립국어원):** gureum

**한자:** 雲

#### 예문

- 흰 **구름**이 흘러갑니다. — 관찰
- **구름** 한 점 없음. — 맑음
- **클라우드** = *cloud*. — 기술 용어

#### 관련어

- ''흐림'' — 형용사
- [[안개]] / ''연무'' — 지표면 구름
- ''먹구름'' — 비 구름

#### 문화적 배경

"구름" = 클라우드 컴퓨팅. "구름 위를 걷다" = 황홀함. "구름 잡다" = 막연한 일. "구름 타고 가다" = 자유로움.

#### 출처

- [[nature-vocabulary]]

---

### 비

**품사:** 명사

**정의:** 구름에서 떨어지는 물방울.

**로마자 (국립국어원):** bi

**한자:** 雨

#### 예문

- **비**가 옵니다. — 예보
- **비**가 많이 옵니다. — 양
- **비** 오나 안 오나. — 관용구: 상관없이

#### 관련어

- ''비오다'' — 동사
- ''이슬비'' — 약한 비
- ''장대비'' — 센 비
- ''강수량'' — 측정

#### 문화적 배경

"비" = 축복(농업) 또는 우울(기분). "비 온 뒤 땅 굳는다" = 고난 후 기반 다짐. "비 맞다" = 고생하다. 장마 = 한국 여름 우기.

#### 출처

- [[nature-vocabulary]]

---

### 눈

**품사:** 명사

**정의:** 얼어붙은 결정질 물이 조각으로 내리는 것.

**로마자 (국립국어원):** nun

**한자:** 雪

#### 예문

- **눈**이 쌓입니다. — 명사
- **눈**이 옵니다. — 동사(오다)
- **눈** 나라. — 설국

#### 관련어

- ''눈오다'' — 동사
- ''눈사람'' — 눈 조각
- ''첫눈'' — 계절 첫 눈
- ''폭설'' — 많은 눈

#### 문화적 배경

"눈" = 겨울 기쁨(썰매) 또는 부담(제설). "눈덩이처럼 불어나다" = 기하급수적 증가. "눈물" = 눈+물 = 슬픔. "눈 감다" = 죽다/외면하다.

#### 출처

- [[nature-vocabulary]]

---

### 바람

**품사:** 명사

**정의:** 움직이는 공기. 자연스러운 수평 기류.

**로마자 (국립국어원):** baram

**한자:** 風

#### 예문

- **바람**이 붑니다. — 관찰
- **풍력** 발전 = 바람 에너지. — 재생가능
- **바람** 맞다 = 감기 걸리다. — 관용구

#### 관련어

- ''바람불다'' — 동사
- ''미풍'' — 약한 바람
- ''강풍'' — 센 바람
- ''태풍'' — 열대 저기압

#### 문화적 배경

"바람" = 변화("바람이 분다"). "바람 피다" = 외도. "바람나다" = 마음이 변하다. "바람 잘 날 없다" = 늘 시끄러움. "봄바람" = 설렘.

#### 출처

- [[nature-vocabulary]]

---

### 폭풍

**품사:** 명사

**정의:** 바람, 비, 천둥, 번개, 눈을 동반한 대기 대란.

**로마자 (국립국어원):** popung

**한자:** 暴風

#### 예문

- **폭풍**이 몰려옵니다. — 경고
- **폭풍**처럼 지나갔습니다. — 비유: 빠르게
- **뇌우** = 천둥번개. — 관련

#### 관련어

- ''폭풍우'' — 비+바람
- ''눈폭풍'' — 눈+바람
- ''태풍'' — 열대 폭풍

#### 문화적 배경

"폭풍" = 격동의 은유. "폭풍 전야" = 직전의 고요. "폭풍 속의 등불" = 위태로움. "폭풍우 치는 밤에" = 김유정 소설.

#### 출처

- [[nature-vocabulary]]

---

### 천둥

**품사:** 명사

**정의:** 번개 뒤따르는 큰 울림 소리.

**로마자 (국립국어원):** cheondung

**한자:** 雷

#### 예문

- **천둥**이 칩니다. — 소리
- **천둥**번개. — 세트
- **천둥**같은 박수. — 비유

#### 관련어

- ''번개'' — 빛
- ''천둥번개'' — 세트
- ''벼락'' — 번개 맞음

#### 문화적 배경

천둥 = 천벌, 신의 분노(전통). "천둥벌거숭이" = 철없는 아이. "벼락 맞다" = 큰 충격/벌. "천둥소리" = 큰 소리.

#### 출처

- [[nature-vocabulary]]

---

## 자연 풍경 (Natural Landscape)

### 산

**품사:** 명사

**정의:** 언덕보다 높고 가파른 지표 자연 융기.

**로마자 (국립국어원):** san

**한자:** 山

#### 예문

- **산**에 오르다. — 등산
- **산맥** = 산줄기. — 연속
- **산악자전거** = *mountain bike*. — 스포츠

#### 관련어

- ''언덕'' — 더 낮음
- ''정상'' / ''봉우리'' — 꼭대기
- ''등산'' — 스포츠

#### 문화적 배경

한국 = 산의 나라(국토 70% 산지). "산 넘어 산" = 어려움 연속. "산삼" = 산에서 나는 삼. "산신령" = 산의 신. 백두산 = 민족 영산.

#### 출처

- [[nature-vocabulary]]

---

### 바다

**품사:** 명사

**정의:** 큰 소금물 덩어리. 바다(sea) vs 대양(ocean).

**로마자 (국립국어원):** bada

**한자:** 海

#### 예문

- **바다**가 잔잔합니다. — 상태
- **해수욕** = *sea bathing*. — 레저
- **해산물** = 바다 음식. — 요리

#### 관련어

- [[대양]] — 더 큼
- ''해변'' / ''해안'' — 경계
- ''해수'' — 소금물
- ''뱃멀미'' — 멀미

#### 문화적 배경

한국 = 삼면이 바다. "바다 건너" = 외국. "바다의 왕" = 용왕. "바다 사나이" = 어부/선원. 해녀 = 제주 여성 잠수부.

#### 출처

- [[nature-vocabulary]]

---

### 강

**품사:** 명사

**정의:** 바다·호수·다른 강으로 흐르는 자연 큰 물줄기.

**로마자 (국립국어원):** gang

**한자:** 川

#### 예문

- **강**이 남쪽으로 흐릅니다. — 방향
- **강**을 건너다. — 행동
- **강변** = 강가. — 산책로

#### 관련어

- ''시내'' — 더 작음
- ''지류'' — 합류하는 강
- ''하구'' — 강 입구
- ''삼각주'' — 델타

#### 문화적 배경

"강" = 생명, 시간("흐르는 강물"). "강물 흐르듯" = 자연스럽게. "강 건너 불구경" = 남의 일 방관. 한강 = 서울 상징. 낙동강 = 최장.

#### 출처

- [[nature-vocabulary]]

---

### 호수

**품사:** 명사

**정의:** 육지로 둘러싸인 물(담수/염수).

**로마자 (국립국어원):** hosu

**한자:** 湖

#### 예문

- **호수**에서 수영. — 레저
- **호숫가** 별장. — 별장
- **호수** 밑. — 바닥

#### 관련어

- ''연못'' — 더 작음
- ''댐호수'' — 인공
- ''호수여행'' — 관광

#### 문화적 배경

"호수" = 휴양, 평화. "호수 같은 마음" = 평온. "호수에 돌 던지기" = 파문 일으키기. 백두산 천지 = 화산호.

#### 출처

- [[nature-vocabulary]]

---

### 섬

**품사:** 명사

**정의:** 물에 완전히 둘러싸인 땅.

**로마자 (국립국어원):** seom

**한자:** 島

#### 예문

- **섬** 여행. — 관광
- **섬나라** = *island nation*. — 국가
- **섬** 투어. — 여행

#### 관련어

- ''무인도'' — 사람 없는 섬
- ''군도'' — 섬들의 모임
- ''반도'' — 거의 섬
- ''환초'' — 산호초 고리

#### 문화적 배경

한국 = 3,300여 개 섬. "섬마을" = 고립된 공동체. "섬 소년" = 순박한 아이. "섬 탈출" = 탈출 게임. 독도 = 동쪽 끝 섬.

#### 출처

- [[nature-vocabulary]]

---

### 나무/숲

**품사:** 명사

**정의:** 나무가 우거진 곳.

**로마자 (국립국어원):** sup / namu

**한자:** 森 / 木

#### 예문

- **숲**을 걷다. — 산림욕
- **나무**가 그늘 만듭니다. — 기능
- **숲속** 집. — 집

#### 관련어

- ''나무'' — 개별
- ''숲속'' — 내부
- ''산림'' — 학술용어
- ''벌목'' — 베기

#### 문화적 배경

"숲" = 신비, 휴식. "숲속의 집" = 동화. "숲 치유" = 산림욕(산림청 인증). "나무 심다" = 미래 투자.

#### 출처

- [[nature-vocabulary]]

---

### 대양

**품사:** 명사

**정의:** 지구 표면 71% 덮는 거대 소금물.

**로마자 (국립국어원):** daeyang

**한자:** 大洋

#### 예문

- **태평양** / **대서양** / **인도양** / **북빙양** / **남빙양**. — 5대양
- **대양** 생물. — 해양 생물
- **대양** 오염. — 환경 문제

#### 관련어

- [[바다]] — 더 작음
- ''해류'' — 순환
- ''심해'' — 깊은 바다
- ''해양학'' — 학문

#### 문화적 배경

"대양" = 궁극의 광대함. "대양의 물 한 방울" = 미미함. "대양 횡단" = 도전.

#### 출처

- [[nature-vocabulary]]

---

## 자연 현상 (Natural Phenomena)

### 얼음

**품사:** 명사

**정의:** 물이 얼어 고체가 된 것.

**로마자 (국립국어원):** eoreum

**한자:** 氷

#### 예문

- **얼음**이 얼었습니다. — 상태
- **얼음** 녹기 전. — 시간
- **얼음장** 같은 추위. — 비유

#### 관련어

- ''얼다'' — 동사
- ''녹다'' — 반대
- ''빙판'' — 얇은 얼음
- ''드라이아이스'' — 고체 CO₂

#### 문화적 배경

"얼음" = 차갑고 단단함. "얼음 깨기" = 어색함 풀기(아이스브레이킹). "얼음 공주" = 차가운 여자. "얼음땡" = 얼음 놀이.

#### 출처

- [[nature-vocabulary]]

---

### 안개

**품사:** 명사

**정의:** 지표 근처 떠다니는 미세 물방울. 시야 가림.

**로마자 (국립국어원):** angae

**한자:** 霧

#### 예문

- **안개** 끼어 앞 안 보임. — 위험
- **안개** 걷히다. — 해제
- **안개** 속 같다. — 불확실

#### 관련어

- ''안개낀'' — 형용사
- ''연무'' — 옅은 안개
- ''스모그'' — 연기+안개

#### 문화적 배경

"안개" = 불확실, 모호함. "안개 속을 걷다" = 앞길 모름. "안개 걷히다" = 진실 드러남. "안개꽃" = 꽃 이름(카스피아).

#### 출처

- [[nature-vocabulary]]

---

### 무지개

**품사:** 명사

**정의:** 비 방울에서 햇빛 굴절로 생기는 색 띠.

**로마자 (국립국어원):** murigae

**한자:** 虹

#### 예문

- 비 온 뒤 **무지개** 떴다. — 자연
- **무지개** 색: 빨주노초파남보. — 7색
- **무지개** 다리. — 희망

#### 관련어

- ''프리즘'' — 물리 도구
- ''스펙트럼'' — 색 영역
- ''무지개빛'' — 형용사

#### 문화적 배경

무지개 = 희망, 약속(노아 방주). 무지개 깃발 = LGBTQ+ 프라이드. "무지개 잡기" = 불가능 추구. "무지개 너머" = 오즈의 마법사.

#### 출처

- [[nature-vocabulary]]

---

## 계절 (Seasons)

### 봄

**품사:** 명사

**정의:** 겨울과 여름 사이 계절. 싹 틔움, 꽃 핌.

**로마자 (국립국어원):** bom

**한자:** 春

#### 예문

- **봄**이 왔습니다. — 계절 시작
- **봄꽃** 구경. — 벚꽃 등
- **봄**맞이 대청소. — 관습

#### 관련어

- ''봄철'' — 봄 기간
- ''입춘'' — 봄 시작(절기)
- ''꽃피다'' — 동사

#### 문화적 배경

한국 = 벚꽃 개화 예보 전국민 관심. "봄나들이" = 소풍. "봄바람" = 설렘. "봄 타다" = 감상적/나른함. 춘분 = 낮밤 같아짐.

#### 출처

- [[nature-vocabulary]]

---

### 여름

**품사:** 명사

**정의:** 봄과 가을 사이. 가장 더운 계절.

**로마자 (국립국어원):** yeoreum

**한자:** 夏

#### 예문

- **여름** 휴가 계획. — 휴가
- **여름** 더위 대비. — 에어컨
- **여름** 장마철. — 비

#### 관련어

- ''여름철'' — 여름 기간
- ''하지'' — 여름 절정(절기)
- ''폭염'' — 극심한 더위
- ''피서'' — 더위 피하기

#### 문화적 배경

한국 여름 = 장마(6-7월) + 폭염(8월). "여름나기" = 더위 견디기. "여름밤" = 열대야. 복날 = 삼복(초복/중복/말복) 보양식(삼계탕).

#### 출처

- [[nature-vocabulary]]

---

### 가을

**품사:** 명사

**정의:** 여름과 겨울 사이. 수확, 단풍, 서늘함.

**로마자 (국립국어원):** gaeul

**한자:** 秋

#### 예문

- **가을** 단풍 구경. — 관광
- **가을** 하늘 높고 말이 살찐다. — 속담
- **가을**걷이 = 수확. — 농사

#### 관련어

- ''가을철'' — 가을 기간
- ''추분'' — 가을 중간(절기)
- ''단풍'' — 나뭇잎 색 변함
- ''추수'' — 수확

#### 문화적 배경

한국 가을 = 여행 최적기. 추석(음력 8월 15일) = 최대 명절. "가을 타다" = 감상적/우울. "하늘 높고 말 살찐다" = 풍요.

#### 출처

- [[nature-vocabulary]]

---

### 겨울

**품사:** 명사

**정의:** 가을과 봄 사이. 가장 추운 계절. 눈, 얼음.

**로마자 (국립국어원):** gyeoul

**한자:** 冬

#### 예문

- **겨울** 준비 완료. — 월동
- **겨울** 스포츠(스키/스케이트). — 레저
- **겨울** 방학. — 학교

#### 관련어

- ''겨울철'' — 겨울 기간
- ''동지'' — 밤 가장 김(절기)
- ''한파'' — 심한 추위
- ''월동'' — 겨울 나기

#### 문화적 배경

한국 겨울 = 건조, 추위(영하 10-20도). 김장(11월) = 겨울 김치 담그기. 동지 팥죽 = 액막이. 설날(음력 1월 1일) = 새해 맞이. "겨울잠" = 동면/휴식.

#### 출처

- [[nature-vocabulary]]

---

## Sources

- TOPIK N4-N5 자연 어휘
- 국립국어원 표준국어대사전
- [[first-travel-japan]] — 한국 여행 중 자연 단어

---

## Pipeline Form (machine-readable)

> Generated for downstream consumers (`Game/typing_language/raw/{lang}_words.md`).
> Schema reference: `wiki/pipeline-to-game.md` L33-39, L92.
> The body above remains the human-readable form and is the source of truth.

```yaml
- { id: 'kr_nature_001', display: '태양', input: '태양', meaning: 'sun', level: 'TOPIK 1', category: 'nature-vocabulary', source: '[[nature-vocabulary]]' }
- { id: 'kr_nature_002', display: '달', input: '달', meaning: 'moon', level: 'TOPIK 1', category: 'nature-vocabulary', source: '[[nature-vocabulary]]' }
- { id: 'kr_nature_003', display: '별', input: '별', meaning: 'star', level: 'TOPIK 1', category: 'nature-vocabulary', source: '[[nature-vocabulary]]' }
- { id: 'kr_nature_004', display: '하늘', input: '하늘', meaning: 'sky', level: 'TOPIK 1', category: 'nature-vocabulary', source: '[[nature-vocabulary]]' }
- { id: 'kr_nature_005', display: '구름', input: '구름', meaning: 'cloud', level: 'TOPIK 1', category: 'nature-vocabulary', source: '[[nature-vocabulary]]' }
- { id: 'kr_nature_006', display: '비', input: '비', meaning: 'rain', level: 'TOPIK 1', category: 'nature-vocabulary', source: '[[nature-vocabulary]]' }
- { id: 'kr_nature_007', display: '눈', input: '눈', meaning: 'snow', level: 'TOPIK 1', category: 'nature-vocabulary', source: '[[nature-vocabulary]]' }
- { id: 'kr_nature_008', display: '바람', input: '바람', meaning: 'wind', level: 'TOPIK 1', category: 'nature-vocabulary', source: '[[nature-vocabulary]]' }
- { id: 'kr_nature_009', display: '폭풍', input: '폭풍', meaning: 'storm', level: 'TOPIK 2', category: 'nature-vocabulary', source: '[[nature-vocabulary]]' }
- { id: 'kr_nature_010', display: '천둥', input: '천둥', meaning: 'thunder', level: 'TOPIK 2', category: 'nature-vocabulary', source: '[[nature-vocabulary]]' }
- { id: 'kr_nature_011', display: '산', input: '산', meaning: 'mountain', level: 'TOPIK 1', category: 'nature-vocabulary', source: '[[nature-vocabulary]]' }
- { id: 'kr_nature_012', display: '바다', input: '바다', meaning: 'sea', level: 'TOPIK 1', category: 'nature-vocabulary', source: '[[nature-vocabulary]]' }
- { id: 'kr_nature_013', display: '강', input: '강', meaning: 'river', level: 'TOPIK 1', category: 'nature-vocabulary', source: '[[nature-vocabulary]]' }
- { id: 'kr_nature_014', display: '호수', input: '호수', meaning: 'lake', level: 'TOPIK 2', category: 'nature-vocabulary', source: '[[nature-vocabulary]]' }
- { id: 'kr_nature_015', display: '섬', input: '섬', meaning: 'island', level: 'TOPIK 2', category: 'nature-vocabulary', source: '[[nature-vocabulary]]' }
- { id: 'kr_nature_016', display: '나무/숲', input: '나무/숲', meaning: '', level: 'TOPIK 1-2', category: 'nature', source: '[[nature-vocabulary]]' }
- { id: 'kr_nature_017', display: '대양', input: '대양', meaning: 'ocean', level: 'TOPIK 2', category: 'nature-vocabulary', source: '[[nature-vocabulary]]' }
- { id: 'kr_nature_018', display: '얼음', input: '얼음', meaning: 'ice', level: 'TOPIK 1', category: 'nature-vocabulary', source: '[[nature-vocabulary]]' }
- { id: 'kr_nature_019', display: '안개', input: '안개', meaning: 'fog', level: 'TOPIK 2', category: 'nature-vocabulary', source: '[[nature-vocabulary]]' }
- { id: 'kr_nature_020', display: '무지개', input: '무지개', meaning: 'rainbow', level: 'TOPIK 2', category: 'nature-vocabulary', source: '[[nature-vocabulary]]' }
- { id: 'kr_nature_021', display: '봄', input: '봄', meaning: 'spring', level: 'TOPIK 1', category: 'nature-vocabulary', source: '[[nature-vocabulary]]' }
- { id: 'kr_nature_022', display: '여름', input: '여름', meaning: 'summer', level: 'TOPIK 1', category: 'nature-vocabulary', source: '[[nature-vocabulary]]' }
- { id: 'kr_nature_023', display: '가을', input: '가을', meaning: 'autumn', level: 'TOPIK 1', category: 'nature-vocabulary', source: '[[nature-vocabulary]]' }
- { id: 'kr_nature_024', display: '겨울', input: '겨울', meaning: 'winter', level: 'TOPIK 1', category: 'nature-vocabulary', source: '[[nature-vocabulary]]' }
```
