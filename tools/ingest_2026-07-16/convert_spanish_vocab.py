#!/usr/bin/env python3
"""
Convert Spanish vocabulary files to full AGENTS.md format with sections per word.
"""
            ...
            for line in lines:
                ...
                if line.startswith('|') and line.endswith('|') and '---' not in line:
                    ...
                    if match:
                        ...
                        if word and word not in ['Español', 'English', 'Palabra', 'Pronunciación', 'Significado', 'Categoría gramatical', 'Connotación', 'Ejemplo', 'Kanji', 'Kana', 'Romaji', 'Meaning']:
                            words.append({
                                'word': word,
...
def extract_words_from_sections(content: str) -> List[Dict]:
    ...
    words = []
    # Pattern for ### word sections
    section_pattern = re.compile(r'(###\s+(.+?))\n\n\*\*Part of Speech:\*\*\s+(.+?)\n\n\*\*Definition:\*\*\s+(.+?)(?:\n\n\*\*IPA:\*\*\s+(.+?))?(?:\n\n#### Examples\n\n(.+?))?(?=\n\n---\n\n###|\n\n---\n\n##|\n\n---\n\n## Sources|\n\n---\n\n## Pipeline Form|\Z)', re.DOTALL)
    ...
    for match in section_pattern.finditer(content):
        ...
        ipa = match.group(5).strip() if match.group(5) else ''
        examples = match.group(6).strip() if match.group(6) else ''
        ...
        words.append({
            ...
            'ipa': ipa,
            'examples': examples,
            'section': ''  # We'll determine from context
        })
    return words
...
def infer_pos(word: str, meaning: str) -> str:
    ...
    if any(v in meaning_lower for v in ['to ', 'action', 'verb', 'verbo']):
        return 'verbo'
    if any(v in meaning_lower for v in ['adjective', 'adjetivo', 'describing']):
        return 'adjetivo'
    if any(v in meaning_lower for v in ['adverb', 'adverbio', 'manner']):
        return 'adverbio'
    if any(v in meaning_lower for v in ['preposition', 'preposición', 'conjunction', 'conjunción']):
        return 'preposición/conjunción'
    if any(v in meaning_lower for v in ['interjection', 'interjección', 'exclamation']):
        return 'interjección'
...
def convert_file_es(input_path: Path, output_path: Path):
    ...
    # Extract words - try table format first
    words = extract_words_from_tables(content)
    
    # If no table words, try section format
    if not words:
    ...
    for section_name, section_words in sections_dict.items():
        ...
        for w in section_words:
            ...
            new_lines.append('')
            new_lines.append(f'**Etymology:** {generate_etymology_es(word, meaning)}')
            new_lines.append('')
